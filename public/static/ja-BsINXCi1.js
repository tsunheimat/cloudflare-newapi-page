const e = {
  ...
  "，点击更新": "、クリックして更新してください",
  "。在契约恢复之前无法生成视频。": "",
  "（共 {{total}} 个，省略 {{omit}} 个）": "(合計{{total}}、{{omit}}省略)",
  "（共 {{total}} 个）": "(合計{{total}})",
  "（可自动）": "",
  "（当前仅支持易支付接口，默认使用上方服务器地址作为回调地址！）": "(現在、Easy Payment インターフェイスのみがサポートされており、上記のサーバー アドレスがデフォルトでコールバック アドレスとして使用されます。)",
  ...
  "{{count}} 项操作_other": "{{count}} 操作",
  "{{discount}}折": "価格比 {{discount}}/10",
  "{{discount}}折 · 省 {{percent}}%": "{{discount}}/10 の価格 · {{percent}}% お得",
  "{{inputDesc}} + {{outputDesc}}{{extraServices}} = {{symbol}}{{total}}": "{{inputDesc}} + {{outputDesc}}{{extraServices}} = {{symbol}}{{total}}",
  "{{label}} {{price}} / 1M tokens": "{{label}} {{price}} / 100万トークン",
  "{{min}} - {{max}} 秒": "",
  ...
  "「用户可选」决定用户创建令牌时能否自主选择该分组。未勾选的分组只能由管理员在后台分配给用户，不会出现在用户的令牌创建页面中。": "「ユーザー選択可」はトークン作成時にユーザーがこのグループを選択できるかを制御します。未チェックのグループは管理者のみが割り当て可能で、トークン作成画面には表示されません。",
  "/ 1{{unit}} tokens": "",
  "/ 1M 影片补全 tokens": "",
  ...
  "$/次": "ドル/回",
  "0 - 最低": "0 - 最低",
  "0 表示不设上限": "0は上限なしを意味します",
  "0 表示不限": "0 は無制限を意味します",
  "0 表示无限次": "0は回数無制限を意味します",
  "0.002-1之间的小数": "0.002～1の小数",
  "0.1以上的小数": "0.1以上の小数",
  "0=周日 1=周一 2=周二 3=周三 4=周四 5=周五 6=周六": "0=日曜日 1=月曜日 2=火曜日 3=水曜日 4=木曜日 5=金曜日 6=土曜日",
  "1:1 正方形": "1:1 正方形",
  ...
  "1M图片输入": "1M画像入力",
  "1M缓存创建": "1M キャッシュの作成",
  "1M输入（不含缓存）": "1M入力（キャッシュを除く）",
  "1M输入（含缓存）": "1M入力（キャッシュ含む）",
  "1M输出": "1M出力",
  "2 - 低": "2 - 低い",
  "2:3 竖向": "2:3 縦長",
  "2. 管理员在用户管理中将用户分配到对应分组（如 vip）": "2. 管理者がユーザー管理でユーザーをグループに割り当て（例：vip）",
  '2025年5月10日后添加的渠道，不需要再在部署的时候移除模型名称中的"."': "2025年5月10日以降に追加されたチャネルでは、デプロイ時にモデル名から「.」を削除する必要はなくなりました。",
  "3:2 横向": "3:2 横長",
  "3. 用户创建令牌时，下拉菜单中出现勾选了「用户可选」的分组": "3. トークン作成時、「ユーザー選択可」がチェックされたグループがドロップダウンに表示",
  "30天": "",
  "32位密钥，敏感信息不显示": "32ビットキー、機密情報は表示されません",
  "360智脑": "360 の知的な頭脳",
  "4. 令牌使用所选分组的倍率进行计费": "4. トークンは選択したグループのレートで課金",
  "5 - 正常（默认）": "5 - 正常（デフォルト）",
  ...
  "anthropic-beta JSON 示例": "anthropopic-beta JSON の例",
  "API Key": "API Key",
  ...
  "API 密钥": "APIキー",
  "API 密钥 (沙盒)": "APIキー（サンドボックス）",
  "API 密钥 (生产)": "APIキー（本番用）",
  "API 密钥（测试环境）": "APIキー（テスト環境）",
  "API 密钥（生产环境）": "APIキー（本番環境）",
  "API 指南": "API ガイド",
  "API 文档": "APIドキュメント",
  "API 私钥": "API秘密鍵",
  "API 私钥（测试环境）": "API秘密鍵（テスト環境）",
  "API 私钥（生产环境）": "API秘密鍵（本番環境）",
  "API 配置": "API設定",
  API令牌管理: "APIトークン管理",
  API使用记录: "API利用履歴",
  API信息: "API情報",
  ...
  "DALL·E 3 横向": "DALL·E 3 横長",
  "DALL·E 3 竖向": "DALL·E 3 縦長",
  "default 和 vip 只能由管理员在「用户管理」中分配给用户。适用于按用户等级定价、内部测试等不希望用户自主选择的场景。": "default と vip は管理者が「ユーザー管理」で割り当てるのみ可能です。ユーザー等級別の料金設定やテストなど、ユーザーに自主選択させたくないシーンに適しています。",
  ...
  "JSON 必须是对象": "JSON はオブジェクトである必要があります",
  "JSON 文本": "JSONテキスト",
  "JSON 无效": "無効なJSON",
  "JSON 格式参考": "JSONリファレンス",
  "JSON 模式": "JSONモード",
  ...
  "OAuth 配置错误：授权端点必须是完整的 URL（以 http:// 或 https:// 开头）": "OAuth 構成エラー: 認可エンドポイントは完全な URL (http:// または https:// で始まる) である必要があります",
  OIDC: "OIDC",
  "OIDC ID": "OIDC ID",
  ...
  "Pay Method Name": "支払い方法名",
  "Pay Method Type": "支払い方法の種類",
  "Ping间隔（秒）": "Ping間隔（秒）",
  Playground: "",
  "POST 参数": "POSTパラメータ",
  ...
  "RSA 私钥 (沙盒)": "RSA 秘密鍵 (サンドボックス)",
  "RSA 私钥 (生产)": "RSA 秘密キー (本番)",
  ...
  "User Prompt Prefix": "ユーザープロンプトプレフィックス",
  ...
  "Waffo API 参数，可空，例如：CREDITCARD,DEBITCARD（最多64位）": "Waffo API パラメータ。空にすることもできます。例: CREDITCARD、DEBITCARD (最大 64 ビット)",
  "Waffo API 参数，可空（最多64位）": "Waffo API パラメータ、空にすることができます (最大 64 ビット)",
  "Waffo Pancake": "ワッフォパンケーキ",
  "Waffo Pancake 设置": "ワッフォパンケーキの設定",
  "Waffo 充值": "ワッフォリチャージ",
  "Waffo 充值的最低数量，默认 1": "Waffo リチャージの最小量、デフォルトは 1",
  "Waffo 公钥 (沙盒)": "Waffo 公開鍵 (サンドボックス)",
  "Waffo 公钥 (生产)": "Waffo 公開キー (本番)",
  "Waffo 公钥（测试环境）": "Waffo 公開鍵 (テスト環境)",
  "Waffo 公钥（生产环境）": "Waffo 公開キー (運用環境)",
  "Waffo 商户 ID": "ワッフォの販売者ID",
  "Waffo 当前使用 USD 结算": "Waffo は現在 USD 決済を使用しています",
  "Waffo 是一个支付聚合平台，支持多种支付方式。": "Waffo は、複数の支払い方法をサポートする支払い集約プラットフォームです。",
  "Waffo 设置": "ワッフォの設定",
  ...
  "Webhook 公钥": "Webhook公開鍵",
  "Webhook 公钥（测试环境）": "Webhook公開鍵（テスト環境）",
  "Webhook 公钥（生产环境）": "Webhook公開鍵（本番環境）",
  ...
  'X 也可以小于 1，当折扣用。想做"只给输出加价"或"额外加固定费用"，请直接写完整计费公式。': "X は 1 未満にすることもでき、割引として使用されます。 「出力に価格のみを追加する」または「追加の固定料金を追加する」場合は、完全な請求式を直接記述してください。",
  一个平台管理多种企业服务: "Manage enterprise services in one platform",
  一个月: "1ヶ月",
  一天: "1日",
  一小时: "1時間",
  一次性余额充值: "1回限りの残高チャージ",
  "一次性支付，付款后自动返回": "1回払い、支払い後に自動的に返金される",
  ...
  上一步: "前へ",
  上一页: "前のページ",
  "上传 PNG/JPG/SVG 图片，建议尺寸 ≤ 128×128px": "PNG/JPG/SVG 画像をアップロードします。推奨サイズ ≤ 128×128px",
  上传参考图: "参考画像をアップロードする",
  上传后将在这里预览参考图: "追加した参考画像はここにプレビューされます",
  上传图片: "写真をアップロードする",
  上传失败: "アップロードに失敗しました",
  上传微信群二维码图片: "WeChatグループのQRコード画像をアップロード",
  "上次保存: ": "最終保存： ",
  上次检测到可加入模型: "最後に検出された結合可能なモデル",
  上次检测时间: "最終検出時間",
  上游价格同步: "上流の価格同期",
  上游倍率同步: "アップストリーム倍率同期",
  上游已删除的模型: "上流の削除されたモデル",
  上游模型管理: "上流モデル管理",
  ...
  下载文件: "",
  下载日志: "Download Logs",
  下载管理员发布的安装包和资料: "インストールパッケージと管理者が公開している情報をダウンロードします。",
  "下面展示这个模型保存后会写入哪些后端字段，便于和原始 JSON 编辑框保持一致。": "保存後にこのモデルでどのバックエンド項目に書き込まれるかを以下に表示します。元の JSON エディタとの整合確認に便利です。",
  "下面是带注释的示例，仅用于参考；实际保存时请删除注释。": "以下は、参照のみを目的とした注釈付きの例です。実際に保存する際には注釈を削除してください。",
  不会出现: "表示されません",
  不再提醒: "今後表示しない",
  不勾选用户可选: "ユーザー選択不可",
  不升级: "アップグレードしない",
  ...
  不符合返利条件: "",
  不设上限: "上限なし",
  "不配置特殊倍率时：": "特殊レートを設定しない場合：",
  "不配置规则时，所有用户看到的下拉框一样：": "ルール未設定時、全ユーザーに同じドロップダウンが表示されます：",
  不重置: "リセットしない",
  不重试: "リトライしない",
  不限: "無制限",
  不限制: "制限なし",
  ...
  "两个分组都勾选了「用户可选」，所以用户创建令牌时可以看到这两个选项：": "両方のグループに「ユーザー選択可」がチェックされているため、トークン作成時にこの2つが表示されます：",
  ...
  "为什么 default 不勾选「用户可选」？": "なぜdefaultに「ユーザー選択可」をチェックしないのか？",
  ...
  为模型前缀覆盖: "モデルの接頭辞をオーバーライドする",
  "为特定用户分组配置可用分组的增减规则。「添加」为该分组新增可用分组，「移除」移除默认可用分组，「追加」直接追加分组": "特定のユーザーグループに対して利用可能グループの追加・削除ルールを設定します。「追加」は新しいグループを追加、「削除」はデフォルトのグループを削除、「付加」は直接付加します",
  ...
  仅提交已勾选: "送信のみチェックあり",
  仅支持: "対応形式：",
  "仅支持 JPEG / PNG / WebP 格式的图片": "JPEG / PNG / WebP 形式の画像のみ対応しています",
  ...
  仅支持上传图片文件: "画像ファイルのアップロードのみをサポートします",
  仅显示已绑定: "バインド済みのみ表示",
  仅显示矛盾倍率: "競合する倍率のみ表示",
  仅检测上游模型更新: "上流モデルの更新のみを検出する",
  "仅用于开发环境，生产环境应使用 HTTPS": "開発環境専用です。本番環境ではHTTPSを使用してください",
  "仅用于换算，实际保存的是额度": "換算用のみ、実際に保存されるのはクォータです",
  仅用订阅: "サブスクリプションのみ",
  仅用钱包: "ウォレットのみ",
  "仅管理员可用。开启后，当系统定时检测全部渠道发现上游模型变更或检测异常时，将按你选择的通知方式发送汇总通知；渠道或模型过多时会自动省略部分明细。": "管理者のみが使用できます。これをオンにした後、システムがすべてのチャネルを定期的に検出し、上流のモデル変更または検出異常を検出すると、選択した通知方法に従って概要通知が送信されます。チャンネルやモデルが多すぎる場合、一部の詳細は自動的に省略されます。",
  "仅统计成功充值订单，按完成时间归档。": "成功したリチャージ注文のみがカウントされ、完了時間に従ってアーカイブされます。",
  仅自己: "自分だけ",
  仅重置配置: "設定のみリセット",
  今日充值: "今すぐ充電してください",
  今日关闭: "今日は表示しない",
  ...
  今日盈利: "本日の利益",
  今日调用: "本日の呼び出し",
  仍有未处理项: "未処理の項目がまだあります",
  "从 0.5 降到 0.3": "0.5から0.3に引き下げ",
  ...
  从配置文件同步: "設定ファイルから同期",
  从默认列表中去掉一个分组: "デフォルトリストからグループを削除",
  付款完成后将自动回到账户页: "支払いが完了すると、自動的にアカウントページに戻ります",
  付费用户: "有料ユーザー",
  代理佣金已扣减: "代理店手数料が差し引かれています",
  代理分销: "販売代理店",
  "代理分销信息加载中...": "エージェント配布情報を読み込んでいます...",
  "代理分销比例不能大于 100": "エージェントの分配率は 100 を超えることはできません",
  "代理分销比例不能小于 0": "エージェントの分配率を 0 未満にすることはできません",
  代理分销默认配置: "エージェント配布のデフォルト設定",
  代理地址: "プロキシアドレス",
  代理已启用: "プロキシが有効になっている",
  代理设置: "プロキシ設定",
  "代理邀请码、邀请用户和佣金流水": "エージェントの招待コード、招待されたユーザー、手数料の流れ",
  ...
  "令牌分组设为 auto 时，按以下顺序依次尝试选择可用分组，排在前面的优先级更高": "トークングループがautoの場合、以下の順序で利用可能なグループを選択します。上位のグループが優先されます",
  "令牌分组设为 auto 时，系统按优先级顺序自动选择一个可用分组。": "トークングループがautoの場合、システムは優先順位に従って利用可能なグループを自動選択します。",
  ...
  "价格为参考估算，实际计费以模型、渠道为准": "価格は参考目安です。実際の課金はモデル・チャネルにより異なります。",
  价格可视化对比: "価格の視覚的比較",
  价格摘要: "価格概要",
  "价格暂时不可用，请稍后重试": "Price temporarily unavailable, please try again later",
  价格根据用量档位和请求条件动态调整: "料金は使用レベルとリクエスト条件に基づいて動的に調整されます",
  价格模式: "価格モデル",
  "价格模式（默认）": "価格モード（デフォルト）",
  价格清单: "料金表",
  ...
  "企业级稳定，高可用运行": "エンタープライズ級の安定性・高可用性",
  ...
  余额地址: "残高アドレス",
  余额返利: "残高リベート",
  "余额返利直接返到邀请人的钱包余额；套餐返利会正常执行套餐分组升级": "残高リベートは招待者のウォレット残高に直接返されます。パッケージ リベートは通常、パッケージ グループにアップグレードされます。",
  "余额返利直接返到邀请人钱包；套餐返利会按套餐自身规则执行分组升级": "残高リベートは招待者のウォレットに直接返されます。パッケージ リベートは、パッケージ独自のルールに従ってグループ化され、アップグレードされます。",
  作废: "無効化",
  作废于: "無効化日",
  ...
  "你还没有处理{{type}}模型（{{count}}个）。是否仅提交当前已勾选内容？_other": "{{type}} モデル ({{count}}) を扱っていません。現在チェックされている内容のみを送信しますか?",
  你还没有授权任何应用: "",
  佣金余额: "手数料残高",
  "佣金支出 / 净流入": "手数料費用 / 純流入額",
  佣金流水: "コミッション履歴",
  ...
  "例如 €, £, Rp, ₩, ₹...": "例：€, £, Rp, ₩, ₹...",
  "例如 0.5 或 2": "たとえば、0.5 または 2",
  "例如 10 表示新用户首充成功后，代理可获得实付金额的 10%": "たとえば、10 は、新規ユーザーが初めて入金に成功した後、エージェントが実際の支払い額の 10% を受け取ることを意味します。",
  "例如 901": "たとえば、901",
  "例如 anthropic-beta": "たとえば、anthropopic-beta",
  "例如 https://docs.newapi.pro": "例：https://docs.newapi.pro",
  "例如 https://example.com/api/mch/balance": "たとえば、https://example.com/api/mch/balance",
  "例如 https://example.com/api/pay/query": "たとえば、https://example.com/api/pay/query",
  "例如 https://example.com/api/pay/unifiedorder": "たとえば、https://example.com/api/pay/unifiedorder",
  "例如 https://example.com/api/waffo/webhook": "たとえば、https://example.com/api/waffo/webhook",
  "例如 https://example.com/console/topup": "たとえば、https://example.com/console/topup",
  "例如 M1776096808": "例: M1776096808",
  "例如 service_tier": "たとえば、service_tier",
  "例如：": "例：",
  ...
  "例如：github-enterprise": "例：github-enterprise",
  "例如：gpt-4.1-nano,regex:^claude-.*$,regex:^sora-.*$": "例: gpt-4.1-nano,regex:^claude-.*$,regex:^sora-.*$",
  "例如：https://example.com/.well-known/openid-configuration": "例：https://example.com/.well-known/openid-configuration",
  "例如：https://example.com/api/waffo/webhook": "例: https://example.com/api/waffo/webhook",
  "例如：https://example.com/console/topup": "例: https://example.com/console/topup",
  "例如：https://gitea.example.com": "例：https://gitea.example.com",
  "例如：https://yourdomain.com": "例：https://yourdomain.com",
  "例如：MER_xxx": "例: MER_xxx",
  "例如：name、full_name": "例：name、full_name",
  "例如：nginx:latest": "e.g.: nginx:latest",
  "例如：preferred_username、login": "例：preferred_username、login",
  "例如：preview": "例：preview",
  "例如：price_xxx": "例: 価格_xxx",
  "例如：prod_6I8rBerHpPxyoiU9WK4kot": "e.g.: prod_6I8rBerHpPxyoiU9WK4kot",
  "例如：PROD_xxx": "例: PROD_xxx",
  "例如：sk_xxx 或 rk_xxx，留空表示保持当前不变": "例: sk_xxx または rk_xxx、現在の値を変更しない場合は空白のままにします。",
  "例如：STO_xxx": "例: STO_xxx",
  "例如：sub、id、data.user.id": "例：sub、id、data.user.id",
  "例如：whsec_xxx，留空表示保持当前不变": "例: whsec_xxx、現在の値を変更しない場合は空白のままにします。",
  ...
  "例如：清理工具参数，避免上游校验错误": "例: 上流の検証エラーを回避するためにツールのパラメータをクリーンアップします。",
  ...
  保存代理分销默认配置: "エージェント配布のデフォルト構成を保存する",
  保存修改: "変更を保存する",
  保存内容审查配置: "コンテンツレビュー設定を保存する",
  保存分组倍率设置: "グループ倍率設定を保存",
  保存分组相关设置: "グループ関連設定を保存",
  "保存后不会回显，请填写当前环境对应的 API 私钥": "保存後はエコーされません。現在の環境に対応する API 秘密キーを入力してください。",
  "保存后不会回显，请填写当前环境对应的 Stripe API 密钥": "保存後はエコーされません。現在の環境に対応する Stripe API キーを入力してください。",
  "保存后不会回显，请填写测试环境对应的 API 密钥": "保存後はエコーされません。テスト環境に対応するAPIキーを入力してください。",
  "保存后不会回显，请填写测试环境对应的 API 私钥": "保存後はエコーされません。テスト環境に対応する API 秘密キーを入力してください。",
  "保存后不会回显，请填写生产环境对应的 API 密钥": "保存後はエコーされません。本番環境に対応したAPIキーを入力してください。",
  "保存后不会回显，请填写生产环境对应的 API 私钥": "保存後はエコーされません。実稼働環境に対応する API 秘密キーを入力してください。",
  保存备用码: "バックアップコード",
  保存备用码以备不时之需: "万一に備え保存",
  保存失败: "保存に失敗しました",
  ...
  倍率模式: "倍率モード",
  "倍率模式（默认）": "拡大モード (デフォルト)",
  "倍率用于计费乘数，勾选「用户可选」后用户可在创建令牌时选择该分组": "レートは課金倍率です。「ユーザー選択可」をチェックすると、ユーザーがトークン作成時にこのグループを選択できます",
  倍率类型: "倍率タイプ",
  值: "価値",
  "假设再加两个分组 default 和 vip，但不勾选用户可选：": "さらにdefaultとvipの2グループを追加しますが、ユーザー選択可にはチェックしません：",
  偏好设置: "設定",
  停止测试: "テストを停止",
  停止重试: "リトライ停止",
  停用: "無効",
  ...
  元: "CNY",
  充值: "チャージ",
  充值与佣金金额趋势: "リチャージ額と手数料額の推移",
  "充值价格（x元/美金）": "チャージ料金（x CNY/USD）",
  "充值价格必须大于 0": "リチャージ価格は 0 より大きくなければなりません",
  充值价格显示: "チャージ料金表示",
  充值分组倍率: "チャージグループ倍率",
  "充值分组倍率不是合法的 JSON 字符串": "チャージグループ倍率は有効なJSON文字列ではありません",
  充值完成后跳回的页面: "充電完了後に戻るページ",
  充值总额: "合計リチャージ",
  "充值成功，额度已到账": "リチャージが成功し、金額を受け取りました。",
  充值数量: "チャージ額",
  "充值数量，最低 ": "チャージ額、最低",
  充值数量不能大于: "リチャージ金額は以下を超えることはできません",
  ...
  充值确认: "チャージの確認",
  充值类型: "リチャージタイプ",
  "充值类计划对每次充值生效，订阅类计划仅首次生效": "チャージ系プランは毎回のチャージに適用され、サブスク系プランは初回のみ適用されます",
  充值订单: "リチャージ注文",
  充值记录和状态查询: "チャージ記録とステータス照会",
  充值账单: "チャージ履歴",
  充值金额: "チャージ金額",
  充值金额折扣配置: "チャージ額の割引設定",
  ...
  兜底档: "結論",
  入账时间: "入場時間",
  "全场景适配 · 赋能多元业务发展": "あらゆるシーンに適合 · 多様なビジネスの発展を支援",
  "全局控制侧边栏区域和功能显示，管理员隐藏的功能用户无法启用": "サイドバーの表示項目と機能をグローバルに制御します。管理者が非表示にした機能は、ユーザーは有効にできません",
  全局系统提示词拼接: "グローバル システム プロンプトのワード スプライシング",
  全局系统提示词豁免: "グローバルシステムプロンプトの除外",
  全局设置: "グローバル設定",
  全平台: "すべてのプラットフォーム",
  "全方位安全，合规可控": "全方位のセキュリティとコンプライアンス",
  全选: "すべて選択",
  全选当前列表模型: "現在のリスト モデルをすべて選択します",
  全部: "すべて",
  全部供应商: "すべて",
  全部分组: "すべて",
  ...
  全部类型: "すべて",
  全部退出: "",
  "全面适配 Web 开发、小程序、APP、政企系统、第三方平台对接等各类场景，支持多协议与多数据格式，无缝衔接各类业务系统。": "Web開発、ミニプログラム、アプリ、官公庁・企業システム、サードパーティ連携など各種シーンに全面対応。マルチプロトコルと多様なデータ形式をサポートし、各種業務システムとシームレスに連携します。",
  公告: "お知らせ",
  公告内容: "お知らせ内容",
  公告已更新: "お知らせが更新されました",
  ...
  关于系统的详细信息: "システムの詳細情報",
  关于项目: "プロジェクトについて",
  "关键字(ID、名称或卡密)": "キーワード（ID、名称、またはコード）",
  关键调用链路可审计: "",
  关闭: "閉じる",
  关闭侧边栏: "サイドバー折りたたみ",
  关闭公告: "お知らせを閉じる",
  ...
  "内置接口鉴权、访问限流、异常拦截、数据加密等多重安全机制，精准拦截恶意请求，规范接口访问规则，全方位守护企业接口数据与业务安全。": "API認証、アクセスレート制限、異常インターセプト、データ暗号化など多重のセキュリティ機構を内蔵。悪意あるリクエストを的確に遮断し、APIアクセスルールを規範化して、企業のAPIデータと業務の安全を全面的に守ります。",
  "内联图片总大小不能超过 {{limit}}": "",
  "内联图片总大小不能超过 45 MB": "インライン画像の合計サイズは 45 MB 以下にしてください",
  最优可用: "利用可能な最安",
  最低: "最低",
  最低充值数量: "最低チャージ金額",
  "最低充值数量必须大于 0": "最小リチャージ額は 0 より大きい必要があります",
  最低充值美元数量: "最低チャージUSD額",
  ...
  最后登录: "最終ログイン",
  最后请求: "最終リクエスト日時",
  "最多上传 {{count}} 张参考图_other": "最大 {{count}} の参考写真をアップロードします",
  最多只能指定一张尾帧图: "末尾フレーム画像は 1 枚だけ指定できます",
  最多只能指定一张首帧图: "先頭フレーム画像は 1 枚だけ指定できます",
  "最多添加 {{limit}} 个参考视频": "参考動画は最大 {{limit}} 件まで追加できます",
  "最多添加 {{limit}} 个参考音频": "参考音声は最大 {{limit}} 件まで追加できます",
  "最多添加 {{limit}} 张参考图": "参考画像は最大 {{limit}} 枚まで追加できます",
  ...
  最近活动: "",
  最高优先级: "最高優先",
  最高充值不能小于最低充值: "最大リチャージを最小リチャージより小さくすることはできません",
  "最高实付金额（元）": "実際の支払上限額（元）",
  最高档: "トップグレード",
  写: "書込",
  准入策略: "アドミッションポリシー",
  ...
  分佣比例: "手数料率",
  分佣订单数: "コミッション注文数",
  "分时缓存 (Claude)": "タイムシェアリングキャッシュ（クロード）",
  分档价格表: "段階的な価格リスト",
  分类名称: "分類名称",
  分组: "グループ",
  分组JSON设置: "グループJSON設定",
  ...
  分组价格: "グループ料金",
  "分组价格 = 官方价格 × 分组倍率 ÷ 7": "グループ料金 = 公式料金 × グループ倍率 ÷ 7",
  分组倍率: "グループレート",
  "分组倍率 {{ratio}} × 换算 {{rate}} = 相对官方价 {{effective}}": "グループ倍率 {{ratio}} × 換算 {{rate}} = 公式価格の {{effective}}",
  分组倍率设置: "グループ倍率設定",
  ...
  分组名: "グループ名",
  分组名称: "グループ名",
  分组名称到倍率的映射: "グループ名からレートへのマッピング",
  分组描述: "グループの説明",
  "分组是用于控制计费倍率和模型访问权限的核心概念。每个用户属于一个分组，每个令牌也可以指定使用某个分组。": "グループは課金レートとモデルアクセス権限を制御するための中核概念です。各ユーザーは1つのグループに属し、各トークンも特定のグループを指定できます。",
  分组特殊倍率: "グループ特殊レート",
  分组特殊可用分组: "特殊利用可能グループ",
  分组相关设置: "グループ関連設定",
  分组管理: "グループ管理",
  分组维度收入: "グループ別の収入",
  分组维度消耗: "グループ化ディメンションの消費",
  分组设置: "グループ設定",
  分组设置使用说明: "グループ設定ガイド",
  ...
  "删除后无法恢复，且无返利记录的计划才允许删除": "削除を復元することはできません。リベート レコードのないプランは削除できます。",
  ...
  "删除已选 {{selected}} / {{total}}": "選択した {{selected}} / {{total}} を削除します",
  删除成功: "削除に成功しました",
  删除所选: "選択した項目を削除",
  删除所选令牌: "選択したトークンを削除",
  删除所选通道: "選択したチャネルを削除",
  ...
  加载数据分析失败: "データ分析のロードに失敗しました",
  "加载日志中...": "Loading logs...",
  加载日志失败: "ログのロードに失敗しました",
  加载模型价格失败: "",
  ...
  加载邀请信息失败: "招待情報の読み込みに失敗しました",
  "加载隐私政策内容失败...": "プライバシーポリシーのコンテンツの読み込みに失敗しました",
  动态计费: "動的な請求",
  "勾选后，该分组会出现在用户创建令牌时的下拉菜单中。未勾选的分组只能由管理员分配，用户自己无法选择。": "チェックすると、このグループはトークン作成時のドロップダウンに表示されます。未チェックのグループは管理者のみが割り当て可能です。",
  包含: "含む",
  ...
  匹配模式: "一致パターン",
  匹配类型: "マッチングタイプ",
  "区分邀请返利与代理分销带来的充值与激励支出。": "招待リベートとリチャージ、および代理店の配布によってもたらされるインセンティブ支出を区別します。",
  区域: "リージョン",
  升级分组: "アップグレードグループ",
  协助码: "Assistance code",
  ...
  "单张图片上限 {{limit}}": "",
  "单张图片大小不能超过 {{limit}}": "",
  "单张图片大小不能超过 10 MB": "画像 1 枚あたり 10 MB を超えることはできません",
  单条影片价格预估: "",
  占比: "割合",
  卡片视图: "カードビュー",
  危险: "",
  "历史数据已自动按年/月/日/小时汇总，今日尾段实时计算。": "履歴データは年/月/日/時間単位で自動集計され、当日の末尾はリアルタイムで計算されます。",
  历史消耗: "消費履歴",
  原价: "定価",
  "原价，和普通用户一样": "定価、一般ユーザーと同じ",
  原因: "理由",
  "原因：": "理由：",
  原始额度: "元の金額",
  原密码: "現在のパスワード",
  ...
  参照生视频: "参照動画生成",
  参考图: "参考画像",
  "参考图（可选，最多 {{count}} 张）_other": "参考画像 (オプション、最大 {{count}} 画像)",
  "参考图（可选）": "参考画像（任意）",
  "参考图已上传，本次将调用图生视频流程": "",
  "参考影片时长（秒）": "",
  参考生成: "",
  "参考视频（可选）": "参考動画（任意）",
  "参考视频链接（https）": "参考動画のリンク（https）",
  "参考视频链接必须是 https 链接": "参考動画のリンクは https である必要があります",
  参考素材上限: "",
  参考素材生视频: "参照素材から動画生成",
  "参考音频（可选）": "参考音声（任意）",
  "参考音频链接（https）": "参考音声のリンク（https）",
  "参考音频链接必须是 https 链接": "参考音声のリンクは https である必要があります",
  参考音频需要与参考图或参考视频一起使用: "",
  友情链接: "関連リンク",
  发布日期: "公開日",
  发布时间: "公開日時",
  ...
  可以: "Yes",
  "可以根据用户分组增减令牌可选的分组范围。例如 vip 用户额外开放 premium 分组，或移除某个分组的选择权。": "ユーザーグループに基づいて選択可能なグループ範囲を調整できます。例：vipユーザーにpremiumグループへのアクセスを追加、または特定グループの選択権を削除。",
  "可以根据用户分组设置不同的计费倍率。例如 vip 用户使用 standard 令牌时倍率从 1.0 降为 0.8。": "ユーザーグループに基づいて異なる課金レートを設定できます。例：vipユーザーがstandardトークンを使用する際、レートが1.0から0.8に。",
  可信: "信頼できる",
  "可勾选需要执行的变更：新增会加入渠道模型列表，删除会从渠道模型列表移除。": "実行する必要がある変更を確認できます。追加するとチャネル モデル リストに追加され、削除するとチャネル モデル リストから削除されます。",
  可参与次数: "参加可能回数",
  "可参与次数按“单个邀请人”累计命中的不同被邀请用户数计算": "参加可能回数は、「単一招待者」にヒットした異なる招待ユーザーの数に基づいて計算されます。",
  可否选取: "Selectable",
  "可在设置页面设置关于内容，支持 HTML & Markdown": "「このサービスについて」のコンテンツは設定ページで設定でき、HTML & Markdownに対応しています",
  "可填写打款渠道、流水号或备注": "支払いチャネル、シリアル番号、または備考を入力できます",
  ...
  "可选，填写图片 URL": "オプションで、画像の URL を入力します",
  "可选，用于复现结果": "オプション、結果の再現用",
  "可选，用于记录用途": "オプション、ロギング目的",
  "可选，用量达到此档时加收的固定费用": "オプション。使用量がこのレベルに達すると固定料金が請求されます",
  ...
  可选值: "選択可能な値",
  可配置独立密钥管理: "",
  "各渠道成功率 / 失败率": "各チャネルの成功率/失敗率",
  各渠道消耗额度: "各チャネルの消費割り当て",
  "合计：{{total}}": "合計: {{total}}",
  "合计：文字部分 {{textTotal}} + 音频部分 {{audioTotal}} = {{total}}": "合計: テキスト部分 {{textTotal}} + 音声部分 {{audioTotal}} = {{total}}",
  "同一用户可以有多条记录，同一个模型出现在多条记录里也会正常生效。": "同じユーザーが複数のレコードを持つことができ、同じモデルが複数のレコードに出現する場合、通常は有効になります。",
  "同优先级下，充值按更高金额门槛命中；套餐按更贵的触发套餐命中。": "同じ優先度では、リチャージはより高い金額のしきい値に従って行われます。パッケージは、より高価なトリガー パッケージに従ってヒットします。",
  同官方价: "公式価格と同額",
  同时满足: "同時に満足する",
  ...
  启用: "有効にする",
  "启用 BLG 支付": "BLG 支払いを有効にする",
  "启用 Prompt 检查": "プロンプトチェックを有効にする",
  "启用 Waffo": "ワッフォを有効にする",
  "启用 Waffo Pancake": "ワッフォパンケーキを有効にする",
  ...
  启用供应商: "プロバイダーを有効化",
  启用全部: "すべてを有効にする",
  启用后会按测试环境保存这组配置: "有効にした後、この構成セットはテスト環境に従って保存されます。",
  ...
  "启用后将使用 Waffo 沙盒环境": "有効にすると、Waffo サンドボックス環境が使用されます。",
  启用图片生成审查: "イメージ生成レビューを有効にする",
  ...
  启用文本生成内容审查: "テキスト生成コンテンツのレビューを有効にする",
  ...
  "启用首充赠送时，赠送比例必须大于 0": "初回入金ボーナスを有効にする場合、ボーナス比率は 0 より大きくなければなりません",
  启用首次充值赠送: "初回リチャージボーナスを有効にする",
  启用验证: "認証を有効にする",
  周: "週",
  ...
  "命中多个充值门槛时，会按更高门槛的计划优先": "複数のリチャージしきい値に達した場合、しきい値が高いプランが優先されます。",
  命中档位: "ヒットギア",
  命中活动: "ヒットアクティビティ",
  命中率: "ヒット率",
  "命中该亲和规则后，会把此模板合并到渠道参数覆盖中（同名键由模板覆盖）。": "このアフィニティルールがヒットすると、テンプレートがチャネルパラメータオーバーライドにマージされます（同名キーはテンプレートで上書きされます）。",
  和: "と",
  "和Claude不同，默认情况下Gemini的思考模型会自动决定要不要思考，就算不开启适配模型也可以正常使用，如果您需要计费，推荐设置无后缀模型价格按思考价格设置。支持使用 gemini-2.5-pro-preview-06-05-thinking-128 格式来精确传递思考预算。": "クロードとは異なり、ジェミニの思考モデルはデフォルトで考えるかどうかを自動的に決定します。適応機種の電源が入っていなくても通常通りご利用いただけます。課金が必要な場合は、考えられる価格に応じてサフィックスなしのモデルの価格を設定することをお勧めします。思考予算を正確に渡すために、gemini-2.5-pro-preview-06-05- Thinking-128 形式の使用をサポートします。",
  ...
  固定阶梯: "固定はしご",
  "国家/地区": "国/地域",
  "国家/地区信息将用于适用法律法规、监管要求、平台合规策略及模型服务范围的判定。": "国/地域の情報は、適用される法律と規制、規制要件、プラットフォームのコンプライアンス戦略、モデル サービスの範囲を決定するために使用されます。",
  "国家/地区信息已更新": "",
  ...
  "图标文件不能超过 100KB，请压缩后重新上传": "アイコン ファイルは 100KB を超えることはできません。圧縮して再アップロードしてください。",
  图混合: "Blend",
  "图片/音频价格（可选）": "映像・音声価格（オプション）",
  图片倍率: "画像倍率",
  ...
  图片生成失败: "画像生成に失敗しました",
  图片生成完成: "画像生成が完了しました",
  "图片生成审查（作用于 /pg 和 /v1 的 images/generations、images/edits）": "イメージ生成のレビュー (/pg および /v1 のイメージ/生成、イメージ/編集)",
  图片生成模型: "画像生成モデル",
  "图片生成调用：{{symbol}}{{price}} / 1次": "画像生成APIコール：{{symbol}}{{price}} / 1回",
  图片编辑完成: "画像編集完了",
  图片设置: "画像設定",
  "图片读取失败，请重试": "画像の読み込みに失敗しました。もう一度お試しください",
  ...
  图生文: "ディスクライブ",
  图生视频: "画像からの動画生成",
  "在 Stripe 后台创建价格后获得": "Stripe バックエンドで価格を作成した後に取得されます",
  ...
  在线充值活动: "",
  在默认列表基础上新增一个分组: "デフォルトリストに新しいグループを追加",
  "场景：站点提供两个价格档位，用户可以按需选择": "シナリオ：サイトが2つの料金プランを提供し、ユーザーが選択可能",
  "场景：站点有 standard 和 premium 两个用户可选分组。希望 vip 用户额外看到 exclusive 分组，同时不再看到 standard 分组": "シナリオ：standardとpremiumが選択可能。vipユーザーにexclusiveを追加し、standardを非表示にしたい",
  "场景：站点有 standard（倍率 1.0）和 premium（倍率 0.5）两个分组，希望 vip 用户使用 standard 令牌时也能享受折扣": "シナリオ：standard（レート1.0）とpremium（レート0.5）があり、vipユーザーがstandardトークンでも割引を受けられるようにしたい",
  "场景：设置自动选择优先级": "シナリオ：自動選択の優先順位を設定",
  ...
  "填写后覆盖当前测试环境 API 密钥，留空表示保持当前不变": "入力後、現在のテスト環境の API キーを上書きします。現在のものを変更しない場合は、空白のままにします。",
  "填写后覆盖当前测试环境 Webhook 公钥，留空表示保持当前不变": "入力後、現在のテスト環境の Webhook 公開キーを上書きします。現在のものを変更しない場合は、空白のままにします。",
  "填写后覆盖当前测试环境私钥，留空表示保持当前不变": "入力後、現在のテスト環境の秘密キーが上書きされます。現在のものを変更しない場合は、空白のままにします。",
  "填写后覆盖当前生产环境 API 密钥，留空表示保持当前不变": "入力後、現在の本番環境の API キーを上書きします。現在のものを変更しない場合は、空白のままにします。",
  "填写后覆盖当前生产环境 Webhook 公钥，留空表示保持当前不变": "入力後、現在の運用環境の Webhook 公開キーが上書きされます。現在のものを変更しない場合は、空白のままにします。",
  "填写后覆盖当前生产环境私钥，留空表示保持当前不变": "入力後、現在の運用環境の秘密キーが上書きされます。現在のものを変更しない場合は、空白のままにします。",
  "填写后覆盖当前私钥，留空表示保持当前不变": "入力後、現在の秘密キーを上書きします。現在のものを変更しない場合は、空白のままにします。",
  "填写带https的域名，逗号分隔": "https://を含むドメインをカンマ区切りで入力してください",
  "填写平台域名，例如：www.tokenrouter.tech": "プラットフォームのドメイン名を入力します (例: www.tokenrouter.tech)",
  "填写服务器地址后自动生成：": "サーバーアドレスを入力すると自動的に生成されます:",
  "填写测试环境 Waffo 公钥，Base64 或 PEM 内容均可": "テスト環境を入力します。Waffo 公開キー、Base64、または PEM コンテンツを使用できます。",
  "填写生产环境 Waffo 公钥，Base64 或 PEM 内容均可": "運用環境を入力します。Waffo 公開キー、Base64、または PEM コンテンツを使用できます。",
  ...
  处理上游模型更新: "上流モデルの更新を処理する",
  处理中: "Processing",
  处理代理提现: "エージェントの引き出しを処理する",
  处理全部渠道上游更新: "すべてのチャネルのアップストリーム更新を処理する",
  处理提现: "出金の処理",
  备份支持: "バックアップ対応",
  备份状态: "バックアップステータス",
  备注: "備考",
  ...
  复充佣金: "リチャージ手数料",
  复充分成比例: "完全に比例",
  复充指该被邀请用户首充之后的后续成功充值订单: "リチャージとは、招待されたユーザーの最初のリチャージ後に成功したその後のリチャージ注文を指します。",
  复制: "コピー",
  复制代码: "コードをコピー",
  复制令牌: "トークンをコピー",
  复制令牌失败: "トークンのコピーに失敗しました",
  "复制以下提示词发送给 LLM（如 ChatGPT / Claude），让它帮你设计计费表达式": "次のプロンプト ワードをコピーして LLM (ChatGPT / Claude など) に送信すると、請求式の設計に役立ちます。",
  复制全部: "すべてをコピー",
  复制名称: "名称をコピー",
  复制失败: "コピーに失敗しました",
  ...
  "如果你对接的是上游One API或者TokenRouter等转发项目，请使用OpenAI类型，不要使用此类型，除非你知道你在做什么。": "上流の One API に接続する場合、または TokenRouter などのプロジェクトを転送する場合は、OpenAI タイプを使用してください。自分が何をしているのか理解していない限り、このタイプは使用しないでください。",
  ...
  "实时记录接口调用次数、响应状态、异常日志等数据，可视化看板直观展示运行状态，助力快速排查问题、持续优化业务。": "API呼び出し回数、応答ステータス、異常ログなどをリアルタイムに記録。ビジュアルダッシュボードで稼働状況を直感的に表示し、問題の迅速な調査と業務の継続的な最適化を支援します。",
  实时调用监控统计: "リアルタイム呼び出し監視・統計",
  实际: "実績",
  "实际下载尺寸以模型或渠道返回的图片为准，NewAPI 不会压缩或重绘图片": "実際にダウンロードされるサイズはモデルまたはチャネルが返す画像に準じます。NewAPI は画像を圧縮または再描画しません。",
  "实际尺寸：{{size}}": "実際のサイズ：{{size}}",
  "实际尺寸：加载后显示": "実際のサイズ：読み込み後に表示",
  实际模型: "アップストリームモデル",
  实际环境: "実際の環境",
  实际用量: "実使用量",
  ...
  实际请求体: "実際のリクエストボディ",
  实际额度: "実際の金額",
  "审查失败时放行（fallback）": "レビューが失敗した場合のリリース (フォールバック)",
  审计信息: "監査情報",
  客服协助: "Support assistance",
  "客服协助功能当前未开放，暂时无法在网站提交请求，请留意后续开放通知。": "サポート支援は現在利用できないため、ウェブサイトから新しい依頼を送信できません。利用開始のお知らせをご確認ください。",
  ...
  对外运营模式: "公開運用モード",
  对应代理分销已入账佣金: "対応するエージェントの配信に対してコミッションが記録されています",
  "对比：不勾选「用户可选」的场景": "比較：「ユーザー選択可」をチェックしない場合",
  对比项: "比較品",
  ...
  导航菜单: "",
  封禁结束时间必须晚于当前时间: "禁止終了時刻は現在時刻より後でなければなりません",
  ...
  "将把当前编辑中的模型 {{name}} 的价格配置，批量应用到已勾选的 {{count}} 个模型。_other": "現在編集中のモデル {{name}} の価格構成が、選択した {{count}} モデルに一括して適用されます。",
  ...
  "嵌套映射：用户分组 → 使用分组 → 倍率": "ネスト構造：ユーザーグループ → 使用グループ → レート",
  工具标识: "ツールID",
  工具调用定价: "ツール呼び出しの料金",
  工单管理: "作業指示管理",
  左侧边栏个人设置: "サイドバーのアカウント設定",
  "已上传参考图，本次将调用图片编辑接口": "参考画像をアップしました。今回は画像編集インターフェイスが呼び出されます。",
  ...
  "已勾选 {{count}} 个模型_other": "{{count}} モデルがチェックされました",
  已发起支付: "支払いを開始しました",
  ...
  已处理: "Resolved",
  "已处理上游模型更新：加入 {{added}} 个，删除 {{removed}} 个，本次忽略 {{ignored}} 个，当前已忽略模型 {{totalIgnored}} 个": "処理されたアップストリーム モデル更新: {{added}} 追加、{{removed}} 削除、今回は {{ignored}} 無視、モデル {{totalIgnored}} は現在無視",
  已备份: "バックアップ済み",
  已复制: "コピーされました",
  ...
  "已将模型 {{name}} 的价格配置批量应用到 {{count}} 个模型_other": "モデル {{name}} の価格構成が {{count}} モデルにバッチで適用されました",
  已开启: "有効",
  ...
  已打开授权页面: "認可ページを開きました",
  已打开支付页面: "決済ページを開きました",
  "已批量处理上游模型更新：渠道 {{channels}} 个，加入 {{added}} 个，删除 {{removed}} 个，失败 {{fails}} 个": "アップストリーム モデルの更新はバッチで処理されました: チャネル {{channels}}、追加された {{added}}、削除された {{removed}}、失敗した {{fails}}",
  已拉取: "引っ張られた",
  已拒绝: "Rejected",
  已授权应用: "",
  已提交: "送信済み",
  ...
  已注销: "ログアウトしました",
  已消耗额度: "",
  "已消费，但暂未命中返利计划": "利用済みですが、まだ還元プランに該当しません",
  已消费用户数: "消費したユーザーの数",
  已添加: "追加済み",
  ...
  "已清理 {{count}} 个日志文件，释放 {{size}}_other": "{{count}} ログ ファイルがクリーンアップされ、{{size}} がリリースされました",
  已清空: "クリア済み",
  ...
  平台: "プラットフォーム",
  平台价格数据格式不正确: "プラットフォームの価格データ形式が正しくありません",
  平台地址格式不正确: "プラットフォームのアドレス形式が正しくありません",
  "平台将在合规审核通过后予以处理；审核完成前，原登记信息持续有效。提交不真实、不完整或无法核验的材料，平台有权驳回申请，并依据平台规则及适用法律法规采取进一步限制措施。": "プラットフォームはコンプライアンス審査に合格した後に問題を処理します。審査が完了するまで、元の登録情報は有効のままとなります。あなたが虚偽、不完全、または検証不可能な資料を提出した場合、プラットフォームは申請を拒否し、プラットフォームの規則および適用される法律および規制に従ってさらなる制限措置を講じる権利を有します。",
  平均: "平均",
  平均RPM: "平均回転数",
  平均TPM: "平均TPM",
  平均延迟: "平均遅延",
  ...
  开: "On",
  开发者: "開発者",
  "开启「默认使用 auto 分组」后，新建令牌和初始令牌都会自动设为 auto。": "「デフォルトでautoグループを使用」を有効にすると、新規トークンと初期トークンが自動的にautoに設定されます。",
  ...
  "开启后，图片生成页面仅会展示此模型，不再依赖模型名称关键字判断": "有効にすると、画像生成ページにはこのモデルのみが表示され、モデル名のキーワード判定には依存しなくなります。",
  ...
  "开启后，视频生成页面仅会展示此模型，不再依赖模型名称关键字判断": "有効にすると、動画生成ページにはこのモデルのみが表示され、モデル名のキーワード判定には依存しなくなります。",
  ...
  "开启后创建令牌默认选择auto分组，初始令牌也将设为auto": "有効にするとトークン作成時にデフォルトでautoグループが選択され、初期トークンもautoに設定されます",
  开启后未登录用户无法访问模型广场: "有効にすると、ログインしていないユーザーはモデルマーケットプレイスにアクセスできなくなります",
  开启后检测到新增模型会自动加入当前渠道模型列表: "開くと、検出された新しいモデルが現在のチャンネル モデル リストに自動的に追加されます。",
  开启后由后端定时任务检测该渠道上游模型变化: "有効にすると、バックエンドのスケジュールされたタスクは、チャネルの上流モデルの変更を検出します。",
  开启后由模型同时生成配音与音效: "有効にすると、モデルがナレーションと効果音も生成します",
  "开启后该渠道不会参与任何并发竞速，但在竞速全部失败时仍可作为兜底通道被尝试": "有効にすると、このチャネルは並列競走に参加しませんが、競走中のすべてのチャネルが失敗した場合はフォールバックチャネルとして引き続き試行されます",
  开启后该渠道的请求不会被注入全局系统提示词: "有効にすると、このチャネルのリクエストにグローバルシステムプロンプトが注入されません",
  开启批量操作: "一括操作を有効にする",
  开始: "開始",
  开始同步: "同期開始",
  ...
  "当令牌分组设为 auto 时，系统按列表顺序依次选择可用分组。排在前面的优先级更高。": "トークングループがautoの場合、システムはリスト順に利用可能なグループを選択します。上位のグループが優先されます。",
  ...
  "当前代理分销仅针对新用户通过代理邀请码注册后产生的钱包充值订单生效，购买套餐不参与该分销。": "現在のエージェントの配布は、エージェントの招待コードで登録した後に新規ユーザーが生成したウォレット リチャージ注文に対してのみ有効です。パッケージの購入はこの配布には参加しません。",
  "当前代理资格已停用，仅保留历史佣金与流水展示，后续不会再产生新的分销收益": "現在のエージェント資格は無効化され、過去のコミッションとフロー表示のみが保持され、将来的に新たな配信収入は発生しません。",
  当前令牌没有相容分组: "",
  当前价格: "現在の価格",
  当前余额: "現在の残高",
  当前值: "現在の値",
  ...
  "当前分组暂无可用模型，请切换分组或联系管理员配置": "現在のグループには利用可能なモデルがありません。グループを切り替えるか、構成について管理者に問い合わせてください。",
  当前分组没有可用的图片模型: "現在のグループで使用できるイメージ モデルはありません",
  当前分组没有可用的视频模型: "",
  ...
  当前版本: "現在のバージョン",
  当前状态: "Current Status",
  "当前环境共用同一商户 ID": "現在の環境は同じ販売者 ID を共有しています",
  "当前用户将继承后台代理分销的默认首充/复充比例。": "現在のユーザーは、バックエンド エージェント ディストリビューションのデフォルトの初回リチャージ/リチャージ比率を継承します。",
  当前登录账号: "",
  当前登记地区: "現在の登録エリア",
  当前盈利: "経常利益",
  当前绑定: "Bound",
  ...
  "当前设置类型: ": "現在の設定タイプ：",
  "当前账号未开通代理分销，管理员启用后会在这里展示邀请码、分销用户与佣金流水": "現在のアカウントは代理店配信を有効にしていません。管理者が有効化すると、ここに招待コード、配信ユーザー、手数料の流れが表示されます。",
  "当前账户暂不支持用户自行变更国家/地区信息。如确需申请变更，请通过工单提交正式申请，并提供足以证明您当前实际所在国家/地区或主体归属地的真实、完整、有效佐证材料。": "現在のアカウントでは、ユーザーが国/地域情報を自分で変更することはできません。本当に変更を申請する必要がある場合は、作業指示書を通じて正式な申請書を提出し、現在の実際の国/地域または対象者の居住地を証明するのに十分な真実、完全、有効な裏付け資料を提供してください。",
  当前账户未开通代理分销: "現在のアカウントはエージェント配布を開始していません",
  当前跟随系统: "システム設定に準拠",
  当前邀请福利: "現在の招待特典",
  "当前金额未达到 Waffo Pancake 的最低充值要求": "現在の金額がワッフォパンケーキの最低リチャージ要件を満たしていません",
  当前额度: "現在の残高",
  "当某个分组的用户使用另一个分组的令牌时，可设置特殊倍率覆盖基础倍率。例如：vip 分组的用户使用 default 分组时倍率为 0.5": "あるグループのユーザーが別のグループのトークンを使用する際、特殊レートで基本レートを上書きできます。例：vipユーザーがdefaultグループを使用する際のレートを0.5に",
  "当模型没有设置价格时仍接受调用，仅当您信任该网站时使用，可能会产生高额费用": "モデルに料金が設定されていない場合でもAPIコールを受け付けます。信頼できるウェブサイトの場合にのみ使用してください。高額な料金が発生する可能性があります",
  "当用户请求中已有系统提示词时，会优先拼接此内容；当用户未提供系统提示词时，会自动注入该内容": "ユーザーのリクエストにシステム プロンプト ワードがある場合、このコンテンツが最初に結合されます。ユーザーがシステム プロンプト ワードを入力しない場合、コンテンツは自動的に挿入されます。",
  ...
  微信支付: "ウィーチャットペイ",
  微信群: "WeChatグループ",
  微信群二维码: "WeChatグループのQRコード",
  微信群二维码图片链接: "WeChatグループQRコード画像URL",
  "微信账户绑定成功！": "WeChatアカウントの連携に成功しました",
  "必填：请输入服务器地址以自动生成完整端点 URL": "必須: 完全なエンドポイント URL を自動的に生成するには、サーバー アドレスを入力してください。",
  ...
  我的工单: "私の仕事の命令",
  我的订阅: "私のサブスクリプション",
  我确认开启高危重试: "高リスクの再試行を有効にすることを確認します",
  我邀请的: "私が招待しました",
  或: "または",
  ...
  扩展价格: "追加価格",
  扫描二维码: "QRコードスキャン",
  "扫码加入微信群，与大家一起交流使用经验": "QRコードをスキャンしてWeChatグループに参加し、利用経験を共有しましょう。",
  扫码支付: "",
  批量创建: "一括作成",
  ...
  批量检测失败: "バッチ検出に失敗しました",
  "批量检测完成：渠道 {{channels}} 个，新增 {{add}} 个，删除 {{remove}} 个，失败 {{fails}} 个": "バッチ検出が完了しました: チャネル {{channels}}、新しい {{add}}、削除された {{remove}}、失敗した {{fails}}",
  ...
  抽奖券说明: "抽選ルール",
  抽奖活动: "",
  "抽奖活动火热进行中，充值之余别忘了参与抽奖赢取奖励！": "抽選開催中！チャージのついでに参加して報酬を獲得しましょう！",
  "拉取中...": "Pulling...",
  拉取价格: "プル価格",
  拉取平台价格失败: "プラットフォーム価格の取得に失敗しました",
  ...
  拦截原因: "ブロック理由",
  "拦截返回消息（inject_message）": "戻りメッセージをインターセプト (inject_message)",
  "按 1 美元对应的站内价格填写": "1 USD に相当するサイト価格を入力します",
  "按 Cron 表达式定时执行 JS 自动化脚本": "Cron 式に基づいて JS 自動化スクリプトを定期的に実行します。",
  按K显示单位: "K単位で表示",
  "按上游 usage.completion_tokens 结算": "",
  按下单数与已成功订单数计算: "押された注文数と成功した注文数の計算",
  按价格字段筛选: "価格フィールドでフィルタリングする",
  按价格设置: "料金設定",
  按倍率类型筛选: "倍率タイプで絞り込み",
  按倍率设置: "倍率設定",
  按分组与模型估算输入输出成本: "グループおよびモデルごとの入出力コストの見積り",
  按后续成功充值实付金额计算: "リチャージ成功後の実際の支払い額に基づいて計算されます",
  按国家或地区限制模型可见性与调用权限: "国または地域ごとにモデルの可視性と通話許可を制限する",
  按完成支付时间落在当前周期内统计: "現在のサイクル内の支払い完了時間に基づく統計",
  "按实付人民币校验，填 0 表示不限制": "RMB での実際の支払いに基づいて確認します。制限がないことを示すには 0 を入力します。",
  "按应用、令牌、模型的顺序选择后，将配置导入 CC Switch。": "",
  "按当前用户分组归档成功充值收入。": "現在のユーザーグループごとに成功したリチャージ収入をアーカイブします。",
  按成功充值订单口径统计: "成功したリチャージ注文に基づく統計",
  "按成功日志与错误日志的数量口径计算。": "成功ログとエラーログの数に基づいて計算されます。",
  "按成功消费日志汇总各渠道的额度消耗。": "成功した消費ログに従って、各チャネルのクォータ消費を要約します。",
  "按成功消费日志统计模型调用次数。": "成功した消費ログに基づいてモデル呼び出しの数をカウントします。",
  "按所选日期范围联动查看模型调用、延迟、分组收入、拉新贡献和缓存效率。": "モデルの呼び出し、遅延、グループ化された収益、新しい貢献度、およびキャッシュ効率を選択した日付範囲ごとに表示します。",
  按新用户首次充值实付金额计算: "新規ユーザーが最初のリチャージで実際に支払った金額に基づいて計算されます",
  "按日志中的分组字段统计各分组消耗额度。": "各グループの消費クォータは、ログのグループ フィールドに従って計算されます。",
  "按日志响应时间计算各渠道平均延迟和长尾延迟。": "ログ応答時間に基づいて各チャネルの平均遅延とロングテール遅延を計算します。",
  "按日期连续展示当前周期的消费走势。": "現在の期間の消費傾向を日付ごとに継続的に表示します。",
  按模型倍率和分组倍率折算: "モデル倍率とグループ化倍率に応じて換算",
  按次: "リクエストごと",
  ...
  按次价格: "ペイパービュー価格",
  按次计费: "リクエストごとの課金",
  "按消费 / 错误日志去重统计": "消費/エラーログによる重複排除統計",
  "按消费日志中的缓存命中 token 统计": "消費ログのキャッシュ ヒット トークンに基づく統計",
  按消费日志口径统计: "消費ログ口径ごとの統計",
  "按消费日志统计缓存命中、缓存创建与节省额度。": "消費ログに基づくキャッシュ ヒット、キャッシュの作成、および節約に関する統計。",
  "按渠道聚合成功与错误请求，查看稳定性差异。": "成功したリクエストとエラーしたリクエストをチャネルごとに集計して、安定性の違いを確認します。",
  "按照如下格式输入：AccessKey|SecretAccessKey|Region": "Enter in the format: AccessKey|SecretAccessKey|Region",
  按用户创建和首次支付时间统计: "ユーザー作成と最初の支払い時間別の統計",
  "按近几天代理用户的下单与支付结果生成统计视图，成功率按下单数与成功单数计算": "最近のエージェント ユーザーの注文と支払いの結果に基づいて統計ビューを生成します。成功率は、発注数と成功した注文数によって計算されます。",
  "按选定周期内的成功消费额度排序。": "選択した期間の成功した消費量で並べ替えます。",
  按量: "数量に応じて",
  按量计费: "従量課金",
  "按量计费下需要先填写输入价格，才能保存其它价格项。": "従量課金では、他の価格項目を保存する前に入力価格を設定する必要があります。",
  "按页面显示额度判断，保存时自动换算，0 表示不限制": "ページ上に表示されている金額をもとに判断し、保存時に自動換算、0は制限なしを意味します",
  "按页面显示额度封顶，保存时自动换算，0 表示不限制": "金額にはページの表示に応じて上限があり、保存時に自動的に変換されます。0は制限なしを意味します",
  ...
  接收上游模型更新通知: "上流モデルの更新通知を受け取る",
  接收发票邮箱: "Invoice recipient email",
  "控制代理资格、分销比例以及查看当前佣金汇总": "エージェントの資格、分配率を管理し、現在のコミッション概要を表示します",
  控制台: "コンソール",
  控制台区域: "コンソールエリア",
  ...
  "推荐使用（用户可选）": "（オプション）推奨",
  描述: "説明",
  "描述你要生成的画面，例如：赛博朋克城市夜景，霓虹灯，电影感，超清细节": "生成したいシーンを説明します。例: サイバーパンク都市の夜景、ネオンライト、映画のような雰囲気、非常に鮮明な詳細",
  "描述你要生成的视频，例如：镜头缓慢推进雨夜街道，一只橘猫回头望向镜头，电影感光影": "",
  提交: "送信",
  提交和跟进自己的工单: "自分の作業指示を送信してフォローアップする",
  ...
  "搜索用户名、昵称或邮箱": "ユーザー名、ニックネーム、または電子メールを検索",
  "搜索用户名、邮箱或来源订单号": "ユーザー名、電子メール、またはソース注文番号を検索します",
  "搜索用户名、邮箱或订单号": "ユーザー名、メールアドレス、または注文番号を検索します",
  搜索结果: "検索結果",
  搜索聊天应用名称: "チャットアプリ名で検索",
  "搜索规则（描述 / 类型 / 路径 / 来源 / 目标）": "検索ルール (説明/タイプ/パス/ソース/ターゲット)",
  ...
  支付信息: "",
  支付地址: "決済URL",
  "支付处理中，可稍后刷新订阅状态": "支払いが処理されています。サブスクリプションのステータスは後で更新できます。",
  "支付处理中，可稍后在充值记录查看状态": "支払いは処理中です。後でリチャージ記録でステータスを確認できます。",
  支付失败: "支払いに失敗しました",
  "支付失败，请重新发起充值": "支払いに失敗しました。リチャージしてください",
  "支付失败，请重新发起购买": "支払いに失敗しました。購入をやり直してください",
  支付完成单数: "支払い完了数",
  "支付完成后用户跳转的页面，留空则自动使用 服务器地址 + /console/topup": "支払い完了後にユーザーがジャンプするページ。空白のままにすると、サーバー アドレス + /console/topup が自動的に使用されます。",
  支付宝: "Alipay",
  支付宝扫码支付: "Alipayのスキャンコード決済",
  支付成功率: "支払い成功率",
  支付方式: "チャージ方法",
  支付方式分布: "支払い方法の分布",
  支付方式名称: "決済方法名",
  支付方式名称不能为空: "支払い方法名を空にすることはできません",
  支付方式图标: "支払い方法アイコン",
  支付方式类型: "決済方法タイプ",
  支付方式设置: "支払い方法の設定",
  支付方式颜色: "お支払い方法のカラー",
  ...
  "支持 JSONPath，如 email, data.user.email": "Eメール、data.user.emailなどのJSONPathをサポート",
  "支持 JSONPath，如 name, display_name, data.user.name": "name、display_name、data.user.name などの JSONPath をサポートします。",
  "支持 JSONPath，如 preferred_username, login, data.user.username": "JSONPath のサポート (preferred_username、login、data.user.username など)",
  "支持 JSONPath，如 sub, id, data.user.id": "sub、id、data.user.id などの JSONPath をサポート",
  ...
  "支持使用 {prompt} 占位原始提示词": "{prompt} プレースホルダーのオリジナルのプロンプトワードの使用をサポート",
  ...
  支持备份: "バックアップ対応",
  "支持多个标记，使用换行、逗号、分号或竖线分隔。模型输出包含任一标记时将记录审查结果。": "改行、カンマ、セミコロン、または縦棒で区切られた複数のタグがサポートされています。モデル出力にいずれかのフラグが含まれている場合、レビュー結果が記録されます。",
  ...
  "支持文生视频、首尾帧、参考图与参考视频/音频": "テキストからの動画生成、先頭/末尾フレーム、参考画像・動画・音声に対応",
  支持的分辨率: "",
  支持的图像模型: "利用可能な画像モデル",
  支持的时长: "",
  支持的生成模式: "",
  ...
  支持的画面比例: "",
  支持的输出格式: "",
  "支持精确匹配；使用 regex: 开头可按正则匹配。": "正確な一致をサポートします。通常のルールに従って一致させるには、先頭に regex: を使用します。",
  "支持自定义请求限流、频率控制、黑白名单配置，灵活管控接口访问流量，避免恶意请求与突发流量冲击，保障业务有序平稳运行。": "カスタムのレート制限、頻度制御、ブラック/ホワイトリスト設定に対応。APIアクセスのトラフィックを柔軟に管理し、悪意あるリクエストや突発的なトラフィックの衝撃を回避して、業務の秩序ある安定運行を保証します。",
  ...
  文心一言: "ERNIE Bot",
  "文本生成内容审查（命中输出标记时记录审查结果）": "テキスト生成内容のレビュー（出力フラグがヒットしたときに記録されるレビュー結果）",
  文档: "ドキュメント",
  文档中心: "ドキュメントセンター",
  文档加载失败: "ドキュメントの読み込みに失敗しました",
  文档地址: "ドキュメントURL",
  文档导航: "",
  文档搜索加载失败: "ドキュメント検索の読み込みに失敗しました",
  文档目录: "ドキュメント目次",
  文档空间: "",
  文档翻页导航: "",
  文生图: "ヴィンセントの絵",
  文生视频: "テキストからの動画生成",
  新增: "追加",
  "新增 / 首充 / 活跃用户趋势": "新規/初回デポジット/アクティブ ユーザーの傾向",
  "新增 / 首充用户": "新規/初回入金ユーザー",
  "新增 Key 来源": "キーソースを追加",
  新增供应商: "プロバイダーの追加",
  新增失败: "追加に失敗しました",
  ...
  新增封禁: "新しい禁止を追加",
  新增封禁记录: "禁止記録を追加",
  "新增已选 {{selected}} / {{total}}": "選択した {{selected}} / {{total}} を追加",
  新增平台对比: "プラットフォームの比較を追加",
  新增成功: "正常に追加されました",
  新增支付方式: "新しい支払い方法を追加する",
  新增条件: "条件を追加",
  新增模型: "新しいモデルを追加",
  新增用户: "新しいユーザーを追加",
  "新增用户按创建时间统计，首充用户按首次支付时间统计，活跃用户按日志去重统计。": "新規ユーザーは作成時間に基づいてカウントされ、初回入金ユーザーは初回支払い時間に基づいてカウントされ、アクティブ ユーザーはログの重複排除に基づいてカウントされます。",
  新增规则: "ルールを追加",
  新增订阅: "サブスクリプションを追加",
  新增邀请计划: "招待プランを追加",
  新密码: "新しいパスワード",
  ...
  是否检测上游模型更新: "上流モデルの更新を検出するかどうか",
  "是否确认充值？": "Confirm the recharge?",
  是否自动同步上游模型更新: "上流モデルの更新を自動的に同期するかどうか",
  是否自动禁用: "自動的に無効にする",
  ...
  显示调试: "デバッグを表示",
  晚上好: "こんばんは",
  "普通按量/按次直接填价格就行；如果价格要跟请求参数或请求头联动，请切到表达式/阶梯计费。": "通常は、数量/時間あたりの価格を直接入力するだけです。価格をリクエスト パラメーターまたはリクエスト ヘッダーにリンクする必要がある場合は、式/ラダー請求に切り替えてください。",
  ...
  "暂无自动分组，点击下方按钮添加": "自動グループがありません。下のボタンをクリックして追加してください。",
  "暂无自定义 OAuth 提供商": "カスタムOAuthプロバイダーはありません",
  "暂无规则，点击下方按钮添加": "ルールがありません。下のボタンをクリックして追加してください。",
  暂无订阅套餐: "利用可能なサブスクリプションプランがありません",
  暂无订阅记录: "サブスクリプション記録がありません",
  暂无请求数据: "リクエストデータはありません",
  暂无赠送日志: "ギフトログはまだありません",
  暂无邀请码: "招待コードはまだありません",
  暂无邀请计划: "招待予定はまだありません",
  暂无邀请链接: "招待リンクはまだありません",
  暂无项目: "プロジェクトはありません",
  暂无预填组: "事前入力グループはありません",
  暂时还没有代理佣金流水: "代理店手数料フローはまだありません",
  暂时还没有代理充值流水: "エージェントのリチャージフローはまだありません",
  暂时还没有代理邀请用户: "ユーザーを招待するエージェントはまだありません",
  暂时还没有邀请用户: "ユーザーはまだ招待されていません",
  暂时还没有邀请额度流水: "招待枠の譲渡はまだありません。",
  暴露倍率接口: "倍率APIを公開",
  更多: "もっと見る",
  ...
  更新: "更新する",
  "更新 BLG 支付设置": "BLG 支払い設定を更新する",
  "更新 Creem 设置": "Update Creem Settings",
  "更新 Stripe 设置": "Stripe設定の更新",
  "更新 Waffo Pancake 设置": "ワッフォパンケーキの設定を更新する",
  "更新 Waffo 设置": "Waffo 設定を更新する",
  更新SSRF防护设置: "SSRF保護設定の更新",
  更新Worker设置: "Worker設定の更新",
  更新于: "",
  更新令牌信息: "トークン情報の更新",
  更新兑换码信息: "引き換えコード情報を更新",
  更新全局系统提示词拼接: "グローバル システム プロンプトのワード スプライシングを更新する",
  ...
  服务器日志管理: "サーバーログ管理",
  服务显示名称: "サービス表示名",
  "未上传参考图时，默认调用文生图接口": "参照画像がアップロードされていない場合、デフォルトで Vincent 画像インターフェイスが呼び出されます。",
  "未上传参考图时，默认调用文生视频流程": "",
  未产生佣金: "手数料は発生しません",
  ...
  "未找到差异化价格，无需同步": "差別化された価格が見つかりません。同期は必要ありません",
  "未找到差异化倍率，无需同步": "差別化倍率が見つかりません。同期は不要です",
  未找到该文档: "ドキュメントが見つかりません",
  未授权: "未認可",
  未提交: "未送信",
  ...
  本月获得: "今月の獲得",
  本站: "このサイト",
  "本站来源使用该汇率，外部平台可在下方单独配置。": "このサイトのソースではこの為替レートが使用されており、外部プラットフォームは以下で個別に設定できます。",
  "本站汇率（CNY/USD）": "当サイトの為替レート（CNY/USD）",
  "本设备：手机指纹/面容，外接：USB安全密钥": "内蔵：スマートフォンの指紋/顔認証、外部：USBセキュリティキー",
  本设备内置: "このデバイス",
  本页暂无目录: "このページには見出しがありません",
  本页目录: "このページの目次",
  本项目根据: "本プロジェクトは",
  ...
  查单地址: "住所を確認する",
  查看: "詳細",
  "查看 Codex 帐号信息与用量": "Codex アカウント情報と使用状況を表示する",
  "查看 JSON 示例": "JSON の例を表示する",
  查看你可使用的分组: "利用可能なグループを表示",
  查看你可使用的模型: "",
  查看关联部署: "View Associated Deployment",
  查看和筛选充值记录: "リチャージ記録の表示とフィルタリング",
  查看图片: "画像を表示",
  查看图片和文本审查结果: "画像とテキストのレビュー結果を表示する",
  查看图片和文本的审查记录: "画像とテキストのレビュー履歴を表示する",
  查看密钥: "APIキーの詳細",
  查看并退出你账户当前的登录设备: "",
  查看开票: "View invoice",
  查看当前可用的所有模型: "現在利用可能なすべてのモデルの閲覧",
  "查看当前邀请福利，管理邀请列表，并追踪邀请额度流水": "現在の招待特典の表示、招待リストの管理、招待割り当てフローの追跡",
  ...
  查看该应用对你账户的用量: "",
  查看详情: "View Details",
  "查看邀请码、邀请用户、充值流水和佣金流水": "招待コード、招待ユーザー、リチャージフロー、コミッションフローを確認する",
  查询: "検索",
  查询视频任务状态失败: "",
  标准价格: "標準価格",
  标签: "タグ",
  ...
  样本数: "サンプル数",
  核对中: "Under review",
  "核心优势 · 为什么选择 {{name}}": "コアの強み · {{name}} が選ばれる理由",
  ...
  "根据哪个维度的 Token 数量决定落在哪一档": "どの次元のトークンの数によって、どのブラケットに分類されるかが決まります。",
  "根据总用量落在哪个档位，所有 Token 都按该档价格计费": "合計使用量がどのブラケットに該当するかに応じて、すべてのトークンはそのブラケットの価格に従って課金されます。",
  ...
  档位标签: "ギアラベル",
  检查更新: "更新を確認",
  检测全部渠道上游更新: "すべてのチャネルのアップストリーム更新を検出する",
  "检测到 FluentRead（流畅阅读）": "FluentReadが検出されました",
  检测到以下高危状态码重定向规则: "次の高リスクのステータス コード リダイレクト ルールが検出されました",
  ...
  检测失败: "検出に失敗しました",
  "检测完成：新增 {{add}} 个，删除 {{remove}} 个": "検出完了: {{add}} 追加、{{remove}} 削除",
  检测超时: "",
  概览: "概要",
  模型: "モデル",
  "模型 {{model}} 的能力契约不可用": "",
  ...
  "正在同步上游价格，请稍候": "アップストリーム価格を同期しています。お待ちください。",
  "正在同步价格，请稍候": "価格を同期中です。お待​​ちください。",
  "正在处理，请稍候": "処理中です。お待​​ちください。",
  "正在处理大内容...": "大容量のコンテンツを処理中...",
  "正在批量处理，请稍候": "バッチ処理中です。お待​​ちください。",
  "正在批量检测，请稍候": "バッチテストが進行中です。お待​​ちください。",
  正在提交: "送信中",
  "正在构建搜索索引…": "検索インデックスを構築中…",
  "正在构造请求体预览...": "リクエストボディのプレビューを生成中...",
  "正在检测，请稍候": "検出中です。お待​​ちください。",
  "正在测试第 ${current} - ${end} 个模型 (共 ${total} 个)": "モデル ${current} - ${end} 個目をテスト中（合計 ${total} 個）",
  "正在生成 ({{seconds}} s)": "({{seconds}}) を生成中",
  正在跟随最新日志: "Following latest logs",
  "正在跳转 GitHub...": "GitHub にリダイレクトしています...",
  "正在跳转...": "リダイレクト中...",
  正常: "普通",
  "正常情况下，令牌的计费倍率由令牌所选的分组决定。特殊倍率可以根据「用户所在分组」进一步覆盖这个倍率。": "通常、トークンの課金レートは選択したグループで決まります。特殊レートを使えば、ユーザーの所属グループに基づいてこのレートを上書きできます。",
  ...
  此支付方式最高充值金额为: "このお支払い方法の最大リチャージ金額は、",
  "此时用户创建令牌时只能看到 standard 和 premium：": "この場合、ユーザーはトークン作成時にstandardとpremiumのみ表示されます：",
  "此档上限（Token 数）": "このファイルの上限（Token数）",
  ...
  "每个充值单位对应的 USD 金额，默认 1.0": "各リチャージ単位に対応する米ドル額、デフォルトは 1.0",
  "每个分组代表一个价格档位。管理员创建分组后，可以选择哪些档位对用户开放自选。": "各グループは1つの料金プランを表します。管理者がグループを作成後、どのプランをユーザーに公開するか選択できます。",
  "每个对比项都可选择来源（本站/外部平台），再分别选择分组和模型进行可视化对比。各来源按自己的汇率计算。": "各比較項目のソース (当サイト/外部プラットフォーム) を選択し、グループとモデルを選択して視覚的に比較できます。各ソースは独自の為替レートを計算します。",
  "每个档位可设置 0~2 个条件（对 len、p 和 c），最后一档为兜底档无需条件。len 为输入上下文总长度（含缓存），推荐用于阶梯条件。": "各ギアには 0 ～ 2 つの条件 (len、p、c の場合) を設定でき、最後のギアは条件なしの一番下のギアになります。 len は入力コンテキスト (キャッシュを含む) の合計長であり、ラダー条件で推奨されます。",
  "每个档位可设置 0~2 个条件（对 p 和 c），最后一档为兜底档无需条件。": "各ギアには 0 ～ 2 つの条件 (p と c の場合) を設定でき、最後のギアは条件なしの一番下のギアとなります。",
  每个模型使用其最低可用分组倍率: "各モデルで利用可能な最小のグループ倍率を使用します",
  "每个用户最多可创建的令牌数量，默认 1000，设置过大可能会影响性能": "各ユーザーが作成できるトークンの最大数、デフォルト1000。大きすぎるとパフォーマンスに影響する場合があります",
  每位邀请人可参与: "招待者は誰でも参加できます",
  每周: "毎週",
  每天: "毎日",
  ...
  每月: "毎月",
  "每百万 Token 价格": "100万トークンあたりの価格",
  "每美元对应 Token 数": "1米ドルあたりのトークン数",
  每隔多少分钟测试一次所有通道: "すべてのチャネルのテスト間隔（分）",
  "比较模型价格，按供应商、分组和能力快速找到适合你的模型。": "モデル料金を比較し、プロバイダー、グループ、機能から最適なモデルをすばやく見つけます。",
  永不过期: "無期限",
  永久: "永続",
  ...
  "沙盒环境 Waffo API 密钥": "サンドボックス環境 Waffo API キー",
  "沙盒环境 Waffo 公钥 Base64 (X.509 DER)": "サンドボックス Waffo 公開鍵 Base64 (X.509 DER)",
  没有匹配的佣金流水: "一致する手数料のロールオーバーはありません",
  没有匹配的充值流水: "一致するリチャージ金額がありません",
  没有匹配的字段: "一致するフィールドがありません",
  没有匹配的文档: "一致するドキュメントがありません",
  没有匹配的日志条目: "No matching log entries",
  没有匹配的规则: "一致するルールがありません",
  没有匹配的邀请用户: "一致する招待ユーザーがいません",
  没有可用令牌用于填充: "利用可能なトークンがありません",
  没有可用模型: "利用可能なモデルがありません",
  "没有找到与“{{query}}”相关的内容": "「{{query}}」に一致する内容が見つかりませんでした",
  ...
  测活: "Health Check",
  测试: "テスト",
  "测试 Webhook 公钥": "Webhook 公開キーをテストする",
  测试中: "テスト中",
  "测试中...": "テスト中...",
  ...
  测试模式: "Test Mode",
  测试环境: "テスト環境",
  "测试环境 Webhook 验签公钥 Base64": "テスト環境 Webhook 署名検証公開鍵 Base64",
  ...
  渠道密钥信息: "チャネルAPIキー情報",
  渠道密钥列表: "チャネルAPIキーリスト",
  "渠道平均响应 / P95 / P99": "チャンネル平均応答 / P95 / P99",
  ...
  "满足条件时，整单价格乘以 X；如果有多条同时命中，会继续相乘。": "条件が満たされると、注文価格全体が X 倍になります。同時に複数のヒットがある場合、乗算は続行されます。",
  演示站点: "デモサイト",
  演示站点模式: "デモサイトモード",
  ...
  点击处理删除模型: "「プロセス」をクリックしてモデルを削除します",
  点击处理新增模型: "クリックして新しいモデルを処理します",
  ...
  生效: "有効",
  生效中: "発効中",
  "用于“1M输入（含缓存）”估算，示例：0.95。": "「1M 入力 (キャッシュを含む)」の見積もりの​​場合、例: 0.95。",
  "用于API调用的身份验证令牌，请妥善保管": "API呼び出し用の認証トークンです。大切に保管してください。",
  用于切换当前下单和回调校验所使用的环境: "現在の注文とコールバック検証に使用される環境を切り替えるために使用されます。",
  用于唯一标识用户的字段路径: "ユーザーを一意に識別するためのフィールドパス",
  "用于校验 Stripe Webhook 签名，保存后不会回显": "Stripe Webhook 署名の検証に使用され、保存後にエコーされません。",
  "用于校验测试环境的 Waffo Pancake Webhook 签名": "テスト環境を検証するための Waffo Pancake Webhook 署名",
  "用于校验测试环境的 Waffo 回调签名": "テスト環境の検証に使用される Waffo コールバック署名",
  "用于校验生产环境的 Waffo Pancake Webhook 签名": "実稼働環境の Waffo Pancake Webhook 署名検証",
  "用于校验生产环境的 Waffo 回调签名": "運用環境を検証するための Waffo コールバック署名",
  ...
  用户分组的联动作用: "ユーザーグループの連動効果",
  用户分组配置: "ユーザーグループ設定",
  "用户创建令牌 → 选择分组下拉框：": "ユーザーがトークン作成 → グループ選択ドロップダウン：",
  "用户创建令牌时选择的分组，决定该令牌的实际计费倍率。一个用户可以创建多个令牌，使用不同分组。": "トークン作成時に選択するグループが実際の課金レートを決定します。1人のユーザーが複数のトークンを異なるグループで作成できます。",
  用户协议: "ユーザー利用規約",
  用户协议已更新: "ユーザー利用規約が更新されました",
  用户协议更新失败: "ユーザー利用規約の更新に失敗しました",
  用户单次最少可充值的美元数量: "ユーザーが一度にチャージできる最低米ドル額",
  用户可选: "ユーザー選択可",
  用户可选分组: "ユーザー選択可能グループ",
  "用户可选分组的名称和描述（只包含勾选了用户可选的分组）": "ユーザーが選択可能なグループの名前と説明（チェック済みのグループのみ）",
  用户名: "ユーザー名",
  用户名字段: "ユーザー名フィールド",
  "用户名字段（可选）": "ユーザー名フィールド（オプション）",
  用户名或邮箱: "ユーザー名かメールアドレス",
  用户名称: "ユーザー名",
  "用户在充值页面看到的支付方式名称，例如：Credit Card": "リチャージページでユーザーに表示される支払い方法の名前 (例: クレジットカード)",
  用户工单处理: "ユーザーの作業指示の処理",
  ...
  "用户消耗排名 Top 10": "ユーザー消費量ランキングトップ10",
  用户消耗排行: "ユーザー消費ランキング",
  用户消耗趋势: "ユーザー消費推移",
  ...
  用户邮箱: "ユーザーメールアドレス",
  "用时/首字": "所要時間 / 初回トークン",
  "用量分段计价，每一段各自按对应档位价格计费（类似电费阶梯）": "使用料金はセグメントごとに設定されており、各セグメントは対応する価格に従って課金されます (電気料金のはしごに似ています)",
  "用量统计、模型选择、成本优化": "",
  用量范围: "投与量の範囲",
  ...
  "留空则使用 /console/topup?show_history=true": "/console/topup?show_history=true を使用するには空白のままにします",
  留空则使用账号绑定的邮箱: "未入力の場合、アカウントに登録されているメールアドレスが使用されます",
  留空则使用默认回调地址: "デフォルトのコールバック アドレスを使用する場合は空白のままにします",
  "留空则使用默认端点；支持 {path, method}": "未入力の場合、デフォルトのエンドポイントが使用されます。{path, method}に対応しています",
  留空则保持原有密钥: "空欄で既存のキーを保持",
  "留空则自动使用 服务器地址 + /api/waffo/webhook": "サーバーアドレス + /api/waffo/webhook を自動的に使用するには、空白のままにします。",
  留空则自动使用当前站点的默认充值页地址: "空白のままにすると、現在のサイトのデフォルトのリチャージ ページ アドレスが自動的に使用されます。",
  ...
  "留空时默认使用服务器地址作为回调地址，填写后将覆盖默认值": "空白のままにすると、デフォルトでサーバー アドレスがコールバック アドレスとして使用されます。入力するとデフォルト値が上書きされます。",
  "留空表示保留当前密钥，仅在需要更换时重新填写": "現在のキーを保持し、変更する必要がある場合にのみキーを補充するには、空白のままにしておきます。",
  留空表示长期有效: "長期有効性を示す場合は空白のままにしてください",
  "登 录": "ログイン",
  登录: "ログイン",
  登录会话: "",
  登录你的账号: "",
  ...
  "直接追加（和添加类似，但无前缀）": "直接付加（追加と類似、プレフィックスなし）",
  相关: "関連",
  相关产品入口: "",
  相关文档: "",
  相关项目: "関連プロジェクト",
  ...
  "确定要仅检测全部渠道上游模型更新吗？（不执行新增/删除）": "すべてのチャネルの上流モデルの更新のみを検出してもよろしいですか? （追加・削除は行いません）",
  ...
  "确定要删除该提供商吗？": "このプロバイダーを削除してもよろしいですか?",
  ...
  "确定要对全部渠道执行上游模型更新吗？": "すべてのチャネルに対してアップストリーム モデルの更新を実行してもよろしいですか?",
  ...
  "确认删除该邀请计划？": "この招待プランを削除しますか?",
  "确认删除这条模型封禁记录？": "このモデル禁止記録を削除してもよろしいですか?",
  确认取消密码登录: "パスワードログイン無効化の確認",
  "确认后会从可提现佣金扣减，并累计到已提现佣金，同时写入代理佣金流水。": "確認後、出金可能な手数料から差し引かれ、出金可能な手数料に積み立てられ、代理店手数料フローに書き込まれます。",
  确认启用: "有効化を確認",
  确认密码: "パスワード（確認用）",
  确认导入配置: "設定インポートの確認",
  ...
  "示例：claude-opus-4-8 输入价，官方 ￥35.00，Claude lite（特价） ￥3.50": "例：claude-opus-4-8 の入力料金は、公式 ¥35.00、Claude lite（特別価格）¥3.50",
  ...
  端口配置详细说明: "ポート設定の詳細説明",
  端点: "エンドポイント",
  "端点 URL 必须以 http:// 或 https:// 开头：": "エンドポイント URL は http:// または https:// で始まる必要があります。",
  ...
  等待首单消费后判定返利: "最初の購入後、リベートが決定されるまで待ちます。",
  筛选: "フィルター",
  签到最大额度: "チェックイン最大クォータ",
  签到最小额度: "チェックイン最小クォータ",
  ...
  "签到成功！获得": "チェックイン成功！獲得",
  签到设置: "チェックイン設定",
  "简化接口调用，赋能业务高效运转": "API呼び出しをシンプルに、ビジネスの効率的な運用を支援",
  "简单来说：同一个令牌分组，不同等级的用户可以享受不同的价格。": "簡単に言えば：同じトークングループでも、異なる等級のユーザーが異なる価格を享受できます。",
  简洁: "シンプル",
  ...
  管理员: "管理者",
  管理员分配的优惠分组: "管理者が割り当てる割引グループ",
  管理员分配的基础分组: "管理者が割り当てる基本グループ",
  管理员区域: "管理者エリア",
  管理员可视化报表中心: "管理者ビジュアルレポートセンター",
  管理员尚未配置需要展示的分组: "",
  管理员尚未配置需要展示的套餐: "",
  管理员尚未配置需要展示的模型: "",
  管理员暂时未设置任何关于内容: "管理者はまだ「このサービスについて」のコンテンツを設定していません",
  管理员暂未配置微信群二维码: "管理者がWeChatグループのQRコードをまだ設定していません。",
  ...
  管理员未开启支付宝充值: "管理者は Alipay リチャージを有効にしていません。",
  "管理员未开启支付宝充值！": "管理者が Alipay リチャージを有効にしていません。",
  "管理员未开启聚合支付！": "",
  管理员未设置用户可选分组: "管理者がユーザー利用可能なグループを設定していません",
  "管理员给用户分配的分组（如 vip）不仅决定用户身份，还会影响后续两个功能：": "ユーザーに割り当てるグループ（例：vip）はユーザーの身分を決定するだけでなく、以下の2つの機能にも影響します：",
  ...
  "粘贴图片链接（http/https）": "画像リンクを貼り付け（http/https）",
  精确: "完全一致",
  精选: "",
  系统: "システム",
  ...
  索引: "インデックス",
  紧凑列表: "コンパクトリスト",
  "纯字符串会直接覆盖整条请求头，或者点击“查看 JSON 示例”按 token 规则处理。": "純粋な文字列はリクエスト ヘッダー全体を直接カバーするか、[JSON の例を表示] をクリックしてトークン ルールに従って処理します。",
  累计佣金: "累積手数料",
  累计充值金额: "累計チャージ金額",
  累计签到: "累計チェックイン",
  累计获得: "累計獲得",
  累计贡献佣金: "累計寄付手数料",
  "累计返利 {{quota}}（共 {{count}} 次）": "累計還元 {{quota}}（計 {{count}} 回）",
  ...
  统计Tokens: "トークン統計",
  统计周期内代理用户创建的充值订单: "統計期間内にエージェント ユーザーが作成したリチャージ注文",
  统计周期内完成支付的成功金额: "統計期間内に完了した正常な支払い金額",
  统计已重置: "統計がリセットされました",
  统计次数: "リクエスト数統計",
  统计视图: "統計ビュー",
  统计额度: "クォータ統計",
  ...
  "缓存创建 Token": "キャッシュ作成トークン",
  "缓存创建 Token (cc)": "キャッシュ作成トークン (cc)",
  "缓存创建 Tokens": "キャッシュ作成トークン",
  "缓存创建-1h": "キャッシュ作成-1時間",
  "缓存创建-1小时": "キャッシュの作成 - 1 時間",
  "缓存创建-1小时 (cc1h)": "キャッシュの作成 - 1 時間 (cc1h)",
  "缓存创建-5分钟": "キャッシュの作成 - 5 分",
  "缓存创建-5分钟 (cc5)": "キャッシュの作成 - 5 分 (cc5)",
  ...
  "缓存创建价格-1小时": "キャッシュ作成料金 - 1 時間",
  "缓存创建价格-5分钟": "キャッシュ作成の価格 - 5 分",
  ...
  "缓存创建价格：{{symbol}}{{price}} / 1M tokens": "キャッシュ作成価格：{{symbol}}{{price}} / 1M tokens",
  "缓存创建价格（1小时）": "キャッシュ作成料金（1時間）",
  "缓存创建价格（5分钟）": "キャッシュ作成料金（5分）",
  ...
  缓存读取: "キャッシュ読み取り",
  "缓存读取 Token (cr)": "キャッシュ読み取りトークン (cr)",
  ...
  "自动（由模型决定）": "自動（モデルが決定）",
  自动分组: "自動グループ",
  "自动分组auto，从第一个开始选择": "自動グループauto、最初から順に選択",
  自动分组选择: "自動グループ選択",
  自动刷新: "Auto Refresh",
  ...
  "自动表示由模型决定分辨率，不发送尺寸参数": "自動はモデルが解像度を決定し、サイズパラメータを送信しません",
  自动选择: "自動選択",
  ...
  获取代理分销信息失败: "エージェント配布情報の取得に失敗しました",
  获取代理统计失败: "エージェント統計の取得に失敗しました",
  获取令牌密钥失败: "トークンキーの取得に失敗しました",
  获取佣金流水失败: "コミッションフローの取得に失敗しました",
  获取充值流水失败: "補給水の取得に失敗しました",
  ...
  计费: "課金",
  "计费乘数，倍率越低费用越低。例如倍率 0.5 表示半价。": "課金倍率。レートが低いほど費用が低くなります。例：レート0.5は半額を意味します。",
  计费凭证: "課金証憑",
  计费开始: "Billing Start",
  计费摘要: "請求概要",
  计费方式: "課金方式",
  ...
  订阅结算: "サブスクリプション精算",
  订阅说明: "サブスクリプション説明",
  "订阅购买成功，已生效": "サブスクリプションが正常に購入され、有効になりました",
  认证方式: "認証方式",
  "让 AI 接口接入更简单": "",
  ...
  该文档可能已被移动或删除: "このドキュメントは移動または削除された可能性があります",
  ...
  该渠道暂无可处理的上游模型更新: "現在、このチャネルには処理すべきアップストリーム モデルの更新がありません。",
  该渠道未开启上游模型更新检测: "このチャネルではアップストリーム モデルの更新検出が有効になっていません",
  ...
  "请先填写服务器地址，以自动生成完整的端点 URL": "完全なエンドポイント URL を自動的に生成するには、最初にサーバー アドレスを入力してください。",
  "请先开启并填写音频输入价格。": "先に音声入力価格を有効にして入力してください。",
  请先新增模型或从左侧列表选择一个模型: "最初に新しいモデルを追加するか、左側のリストからモデルを選択してください",
  "请先粘贴回调 URL": "まずコールバックURLを貼り付けてください",
  请先输入密钥: "まずAPIキーを入力してください",
  请先选择一个作为模板的模型: "まずテンプレートとして使うモデルを選択してください",
  请先选择一条规则: "まずルールを選択してください",
  请先选择不低于最低额度的充值金额: "最初に最低金額以上のチャージ金額を選択してください",
  请先选择令牌: "",
  "请先选择供应商，再选择分组": "プロバイダーを選択してから、グループを選択してください",
  ...
  "请在 Waffo 后台获取 API 密钥、商户 ID 以及 RSA 密钥对，并配置回调地址。": "Waffo バックエンドで API キー、マーチャント ID、および RSA キーのペアを取得し、コールバック アドレスを構成してください。",
  ...
  请填写封禁原因: "禁止の理由を入力してください",
  "请填写当前环境对应的 Product ID": "現在の環境に対応するプロダクト ID を入力してください",
  "请填写当前环境对应的 Store ID": "現在の環境に対応するストアIDを入力してください",
  "请填写当前环境对应的商户 ID": "現在の環境に対応する販売者 ID を入力してください",
  ...
  请求头: "リクエストヘッダー",
  请求头覆盖: "リクエストヘッダーの上書き",
  "请求尺寸：{{size}}": "リクエストサイズ：{{size}}",
  "请求尺寸是发送给模型的目标值，实际输出以模型或渠道返回为准": "リクエストサイズはモデルに送信する目標値です。実際の出力はモデルまたはチャネルの返却結果に依存します。",
  请求并计费模型: "リクエスト課金モデル",
  ...
  请求频率: "リクエスト頻度",
  请点击我: "こちらをクリック",
  "请确保你的服务可被支付网关回调访问，并在网关侧正确配置白名单与回调地址。": "サービスがペイメント ゲートウェイ コールバックからアクセスできること、およびホワイトリストとコールバック アドレスがゲートウェイ側で正しく構成されていることを確認してください。",
  ...
  "请稍后再来查看，或联系管理员获取入群方式。": "後ほど再度確認するか、管理者に参加方法をお問い合わせください。",
  ...
  '请输入 JSON 格式的密钥内容，例如：\n{\n  "type": "service_account",\n  "project_id": "your-project-id",\n  "private_key_id": "...",\n  "private_key": "...",\n  "client_email": "...",\n  "client_id": "...",\n  "auth_uri": "...",\n  "token_uri": "...",\n  "auth_provider_x509_cert_url": "...",\n  "client_x509_cert_url": "..."\n}': `JSON形式のAPIキーを入力してください。例：
{
  ...
  "请输入 Product ID": "製品IDを入力してください",
  "请输入 Slug": "Slugを入力してください",
  "请输入 Store ID": "ストアIDを入力してください",
  ...
  "请输入密钥！": "APIキーを入力してください",
  请输入密钥前缀或尾号: "",
  "请输入封禁原因，用户会直接看到这段原因": "禁止の理由を入力してください。ユーザーはその理由を直接確認できます。",
...
モデルマッピング形式に対応しています
{
  ...
  请选择返利套餐: "リベートパッケージを選択してください",
  请选择通知方式: "通知方法を選択してください",
  请选择需要封禁的模型: "禁止するモデルを選択してください",
  读取你的基本资料: "",
  读取你的邮箱地址: "",
  调整额度: "残高調整",
  调整额度成功: "残高の調整に成功しました",
  ...
  软错误: "ソフトエラー",
  "轻量化易用，低成本落地": "軽量で使いやすく、低コストで導入",
  较早: "",
  输入: "入力",
  "输入 {{price}} / 1M tokens": "入力 {{price}} / 1M tokens",
  "输入 $/1M tokens": "$/1M トークンを入力してください",
  ...
  "输入 Token": "トークンを入力してください",
  "输入 Token 定价": "トークンの価格を入力する",
  "输入 Token 总量": "トークンの合計金額を入力してください",
  "输入 Token 数": "トークンの数を入力してください",
  "输入 Token 数 (p)": "トークンの数を入力してください (p)",
  "输入 Token 数量，查看按当前配置的预计费用。": "トークンの数を入力すると、現在の構成に基づいた推定コストが表示されます。",
  "输入 Token 数量，查看按当前配置的预计费用（不含分组倍率）。": "トークンの数を入力すると、現在の構成に基づいて推定コストが表示されます (グループ化乗数を除く)。",
  "输入 Token 数量，查看按当前阶梯配置的预计费用。": "トークンの数を入力すると、現在の階層構成に基づいた推定コストが表示されます。",
  "输入 Tokens 阶梯": "トークンラダーに入る",
  ...
  "输出 Token": "出力トークン",
  "输出 Token 定价": "出力トークンの価格設定",
  "输出 Token 数": "トークン数を出力",
  "输出 Token 数 (c)": "トークン数を出力(c)",
  "输出 Tokens 阶梯": "出力トークンラダー",
  ...
  返回文档首页: "ドキュメントのトップへ戻る",
  返回登录: "ログインに戻る",
  返回错误: "エラーを返す",
  返套餐: "返品パッケージ",
  "这个公式比较复杂，下面的简化表单没法完整还原，请在表达式编辑模式下修改。": "この式は比較的複雑なので、以下の簡略化した形式では完全に復元できません。式編集モードで修正してください。",
  ...
  这里会显示生成结果: "生成された結果はここに表示されます",
  这里会显示视频结果: "",
  "这里直接编辑 JSON 对象。适合简单覆盖参数的场景。": "ここでJSONオブジェクトを直接編集します。シンプルなパラメータオーバーライドシナリオに適しています。",
  "这里配置 Waffo 下展示给用户的 Card、Apple Pay、Google Pay 等子支付方式。": "ここでは、Waffo の下でユーザーに表示されるカード、Apple Pay、Google Pay、およびその他のサブ支払い方法を構成します。",
  "这里配置代理用户的默认首充和复充分成比例。单个用户如关闭“使用系统默认比例”，则以用户自身设置为准。": "ここで、代理ユーザーのデフォルトの初回入金額と返済額は比例するように設定されています。 1 人のユーザーが「システムのデフォルト比率を使用」をオフにすると、ユーザー自身の設定が優先されます。",
  进入此档额外收费: "このファイルを入力するには追加料金がかかります",
  进度: "進捗",
  进行中: "進行中",
  ...
  退出设备失败: "",
  退款: "返金",
  "适合任务类等按次收费模型。": "リクエスト単位で課金されるタスク系モデルに適しています。",
  ...
  "适用于统一下单返回 payUrl 的聚合支付网关。当前接入包含：下单、异步回调验签和充值入账。": "統一された注文返却 payUrl に適した集約支払いゲートウェイ。現在のアクセスには、注文の発注、非同期コールバック署名の検証、アカウントへのリチャージが含まれます。",
  ...
  "选择 premium 创建的令牌，调用 API 时费用为 standard 的 50%。": "premiumで作成したトークンでAPI呼び出し時の費用はstandardの50%です。",
  选择使用分组: "使用グループを選択",
  选择充值额度: "チャージ額を選択",
  选择分组: "グループを選択",
  "选择分组，可多选并拖拽调整优先级": "",
  ...
  "选择金额后直接跳转到 Waffo Pancake 结账页，支付完成后会回到账户页。": "金額を選択すると、Waffo Pancake のチェックアウトページに直接ジャンプします。支払いが完了すると、アカウントページに戻ります。",
  "选择预设...": "プリセットを選択...",
  ...
  邀请码: "招待コード",
  邀请码奖励: "招待コード特典",
  邀请码已复制: "招待コードがコピーされました",
  邀请获得额度: "招待特典クォータ",
  邀请计划: "ご招待プラン",
  "邀请计划会在被邀请用户每次充值时向其邀请人返利（仅返利一层，订阅购买仍仅首次）。": "招待プランは招待されたユーザーがチャージするたびに招待者へ還元します（1階層のみ。サブスクリプション購入は初回のみ）。",
  邀请返利: "リベートを招待する",
  邀请返利规则配置: "招待リベートルールの設定",
  邀请链接: "招待リンク",
  邀请链接已复制: "招待リンクがコピーされました",
  ...
  "配置 Waffo 充值时可用的支付方式，保存后在充值页面展示给用户。": "Waffo のリチャージ時に利用できる支払い方法を設定し、リチャージ ページでユーザーに表示するために保存します。",
  "配置 WeChat Server": "WeChatサーバー設定",
  "配置：": "設定:",
  配置价: "",
  "配置到 CC Switch": "",
  "配置各工具的调用价格（$/1K次调用）。按次计费模型不额外收取工具费用。": "各ツールのコール価格 ($/1,000 コール) を設定します。ペイパービューモデルには追加のツール料金はかかりません。",
  "配置后会在所有渠道请求中统一拼接到系统提示词前，可用于全局内容过滤": "構成後、すべてのチャネル要求のシステム プロンプト ワードの前に均一に結合され、グローバル コンテンツ フィルタリングに使用できます。",
  "配置后的效果：": "設定後の効果：",
  ...
  "配置计划名称、启用状态、优先级和备注": "プラン名、アクティベーションステータス、優先度、コメントを設定します",
  配置说明: "設定の説明",
  ...
  键名: "キー名",
  '键的前缀 +: 表示添加，-: 表示移除，无前缀表示追加。值为分组描述（移除时填 "remove"）。': 'キーのプレフィックス +: は追加、-: は削除、プレフィックスなしは付加を意味します。値はグループの説明（削除時は "remove"）。',
  ...
  队列中: "待機中",
  阶: "レベル",
  "阶梯内 Token 数": "ラダー内のトークンの数",
  阶梯判断依据: "ラダー判定基準",
  阶梯序号: "ラダー番号",
  阶梯累进: "はしごの進行",
  阶梯计费: "段階的請求",
  "阶梯计费（表达式解析失败）": "段階的請求 (式の解析に失敗しました)",
  阶梯计费详情: "段階的請求の詳細",
  阶梯配置摘要: "ラダー構成の概要",
  附加条件: "追加条件",
  ...
  首充分成比例: "完全に比例",
  首充用户: "初めてのリチャージユーザー",
  "首充赠送最大额度不能小于 0": "初回入金ギフトの最大額は 0 未満にすることはできません",
  "首充赠送比例不能小于 0": "初回入金ボーナス比率は 0 未満にはできません",
  首充赠送结束时间: "初回入金ボーナスの終了時間",
  "首充赠送起充额度不能小于 0": "初回入金ボーナスの最低入金額は 0 未満にすることはできません",
  "首单已消费，但未命中返利计划": "最初の注文は消費されましたが、リベート プランが選択されていませんでした。",
  首单状态: "初回注文状況",
  首尾帧生成: "",
  首尾生视频: "冒頭・末尾動画生成",
  首帧: "先頭フレーム",
  ...
  验证配置错误: "認証設定のエラー",
  高危操作确认: "リスクの高い業務の確認",
  高危状态码重试风险告知与免责声明Markdown: "高リスクステータスコード再試行リスク通知と免責事項マークダウン",
  高危状态码重试风险确认输入文本: "高リスクステータスコード再試行リスク確認入力テキスト",
  高危状态码重试风险确认项1: "ハイリスクステータスコード リトライリスク確認項目1",
  高危状态码重试风险确认项2: "高リスクステータスコード リトライリスク確認項目2",
  高危状态码重试风险确认项3: "ハイリスクステータスコード リトライリスク確認項目3",
  高危状态码重试风险确认项4: "高リスクステータスコード リトライリスク確認項目4",
  高危状态码重试风险输入不匹配提示: "高リスクステータスコード再試行リスク入力不一致プロンプト",
  高危状态码重试风险输入框占位文案: "高リスクステータスコード再試行リスク入力ボックスのプレースホルダーコピー",
  高并发接口调度: "高並列APIスケジューリング",
  高度: "",
  高效: "",
  高级: "高度な設定",
  "高级套餐，半价优惠": "プレミアムプラン、50%オフ",
  高级文本编辑: "高度なテキスト編集",
  高级设置: "詳細設定",
  高级选项: "高度なオプション",
  ...
  "默认使用 USD 结算": "デフォルトでは米ドル決済が使用されます",
  默认使用auto分组: "デフォルトでautoグループを使用",
  默认使用系统名称: "デフォルトのシステム名称",
  默认助手消息: "こんにちは！何かお手伝いできることはありますか？",
  默认区域: "デフォルトリージョン",
  "默认区域，如: us-central1": "デフォルトリージョン（例：us-central1）",
  默认复充分成比例: "デフォルトの複合体は完全に比例します",
  默认异步回调地址: "デフォルトの非同期コールバック アドレス",
  "默认情况下，所有用户创建令牌时看到的可选分组列表是一样的（即「用户可选」列勾选的分组）。": "デフォルトでは、全ユーザーがトークン作成時に同じ選択可能グループリストを見ます（「ユーザー選択可」列がチェックされたグループ）。",
  默认折叠侧边栏: "サイドバーをデフォルトで折りたたむ",
  默认比例: "デフォルトのスケール",
  默认测试模型: "デフォルトテストモデル",
  默认用户消息: "こんにちは",
  默认补全倍率: "デフォルト補完倍率",
  默认首充分成比例: "デフォルトの最初は完全に比例します",
  ...
  "协助码仅用于客服定位案件，不能用于登录；案件关闭后停止使用，永久案件编号仍可用于历史查询。客服收到协助码后，才能在客服后台定位案件并查看已授权的协助案件及其允许的资料进行排查。客服协助不是网站内的在线客服，网站不提供即时聊天，实际沟通请在微信群完成。": "支援コードは案件の検索専用で、ログインには使えません。案件が終了すると無効になりますが、恒久的な案件番号は履歴検索に使えます。コードを受け取った後、サポートは管理画面で案件を特定し、許可された案件情報とデータを確認できます。これはオンラインカスタマーサービスではなく、サイトにリアルタイムチャットはありません。実際の連絡はWeChatグループで行います。",
  协助定位码已更新: "Assistance locator updated",
  "遇到问题请先在网站提交“请求协助”，提交后会取得永久案件编号和六位协助码（协助定位码），请将该码带到微信群发给客服。": "問題がある場合は、まずサイトで「サポートを依頼」を送信してください。恒久的な案件番号と6文字の支援コードが発行されるので、コードをWeChatグループでサポートに伝えてください。",
  处理截止时间: "Handling deadline",
  案件编号: "Case number",
  "客服协助与工单是两个不同功能，客服协助不是在线客服，也不是工单对话；如需正式提交和跟进工单，请使用工单功能。请在下方描述问题，提交后将协助码带到微信群发给客服。": "サポート支援とチケットは別の機能です。サポート支援はオンラインチャットでもチケットの対話でもありません。正式な提出と追跡にはチケット機能を使用してください。以下に問題を記入し、送信後に支援コードをWeChatグループのサポートへ伝えてください。",
